...
decides the base↔layer hand-off, so it costs one layer recompute (already paid in that round)
rather than a from-scratch closure; the TBox and guard-predicate axes remain open. The
