...
CATALOG: list[dict] = [
    ...
    },
    {
        ...
        "status": STATUS_GAP,
        "reduction_target": (
            "a dateTime VALUE_HOOK (an epoch-based field handle on the dual-leaf value lane, "
            "research/zk-field-native-encoding.md §2.5) — design-only; expression fragment "
            "phase 3 (research/zksparql-fragment-extension.md §5.1)"
        ),
        "note": (
            "No dateTime ordering circuit: no `filter_value_dl_datetime` member is compiled and no "
            "composition member binds a committed dateTime literal to an ordering claim, so the "
            "gate fail-closes. The datetime_*/duration_* comparison functions exist in the "
            "externalized noir_XPath estate but are not wired to a committed leaf. Timezone "
            "normalisation and the indeterminate-comparison case are unspecified for the ZK lane "
            "and must be pinned before a hook lands."
        ),
    },
    {
        "id": "Q10_optional",
