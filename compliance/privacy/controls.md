...
| P-2 | **Data minimisation (Art. 5(1)(c))** — process only what is necessary; don't accumulate incidental personal data. | **IMPL** (engine-side defaults) | The engine does **not** log request bodies/queries by default — request logging (`TraceLayer`) is OFF unless `--verbose` is passed. **[OPUS-4.8] sq-toze.34: and even with `--verbose`, request-log redaction is ON by default** — the logged URI keeps the *path* but replaces the *query string* (where `GET /sparql?query=…` carries the full SPARQL text, possibly PII) with a `<redacted len=N fp=…>` length+non-reversible-fingerprint placeholder; an operator must explicitly pass `--log-full-requests` / `SPARQ_LOG_FULL_REQUESTS=1` to log query text verbatim. **Boundary (honest):** this is log-*content* redaction, not anonymity — the log still records method, endpoint, status, a size signal and timing. Prometheus metrics are **aggregate counters/histograms only** (per-endpoint/status counts, latency buckets, triple/subscription gauges) — no query text, no result rows, no client identifiers. No analytics/telemetry phones home. | `crates/sparq-server/src/redact.rs` (redaction + `RedactingMakeSpan`, default `redact_logs:true`); `crates/sparq-server/tests/log_redaction.rs` (query text absent from the redacted log); `crates/sparq-server/src/http.rs` (`if config.verbose { …TraceLayer }`, default `verbose:false`); `crates/sparq-server/src/metrics.rs` (counters/histogram only) |
...
| P-12 | **Error/log hygiene — no personal data leakage in errors/logs (Art. 5(1)(f) / SOC 2 CC6.7)** — error responses and logs must not echo query text, RDF content, or internal paths. | **PARTIAL** (was overclaimed — corrected per audit F-1/F-2) | **What IS hygienic (verified):** the *runtime* error bodies are generic structured JSON — `"internal server error (panic)"` (`http.rs:1429`), `"query worker panicked"` (`:1963`), `"update worker panicked"` (`:1776`), `"not found"` (`:1374`), `"server is at its concurrent-request limit"` (`:1435`), and budget bodies that name *limits, not data* (`engine_error_response`, `:2076`). Metrics carry no PII (`metrics.rs`). **What is NOT hygienic (the overclaim, now corrected):** several *parse/validation* error bodies echo **caller input** verbatim via the underlying parser diagnostic — `"malformed query: {msg}"` (`:1812`, echoes **SPARQL query text**), `"malformed RDF body: {e}"` / `"malformed RDF/XML body: {e}"` (`:2293`/`:2302`, echo **fragments of the loaded RDF data** via `Graph::load_str`→`e.to_string()`, `crates/sparq-core/src/lib.rs:632`), `"update failed: {e}"` (`:2116`), `"query execution error: {msg}"` (`:2860`). Because the bare server has **no auth by design** (boundary B3), the RDF-body sites can return loaded-data fragments to an **unauthenticated** caller. This is the engine-side gap **PR-G1** (now Medium); the code fix is bead **sq-cz89**, the regression test bead **sq-zg0u**. | Hygienic: `crates/sparq-server/src/http.rs:1429,1776,1963,1374,1435,2076`; `crates/sparq-server/src/metrics.rs`. **Echoes caller input:** `:1812,2293,2302,2116,2860` (+ `crates/sparq-core/src/lib.rs:632`). **No regression test pins the no-echo property** (audit F-3). |
