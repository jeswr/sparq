...
|---|---|---|---|
| **PR-X1** | **ZK/MPC "privacy by cryptography" is NOT yet sound.** The estate models private query proofs / MPC but provides **no** production guarantee pending external audit (`SECURITY.md`; `research/zk-soundness-audit.md`; `research/zk-verifier-reaudit.md` — internal re-audit "sound-as-landed", external cryptographer sign-off still pending **sq-qhy4**). Any privacy claim resting on it would be a high-severity overclaim. | The soundness fix is **cryptographic work** tracked by the ZK remediation beads (epic sq-1s2) + the `cryptoreview` framework — **not** this privacy framework. This register only records that privacy claims are **gated** on it. We do **not** mark it "closed" or claim any privacy benefit from it. **A CI gate now enforces this honesty in code** (sq-toze.35, ENFORCED): `scripts/check-privacy-claims.sh` (HARD job `privacy-claims` in `.github/workflows/docs-quality.yml`, aggregated by `ci-summary`) fails the build on any unqualified ZK/MPC privacy/soundness claim across the outward doc/README/SKILL/site/compliance surface, unless the line carries an inline `privacy-claims-allow: <justification>` marker (the audited allow-list). | **sq-toze.35** (gate — **ENFORCED in CI**) + **sq-qhy4** (external audit) + epic **sq-1s2** (the crypto fixes) |
