...
|---|---|---|---|
| MS-1 | **Unsafe surface is *confined*** — 49 first-party crates carry a source `forbid(unsafe_code)` and 8 are unsafe-bearing (the sets overlap for conditional/test-only cases). | **PASS** | Re-runnable per-crate-deduped commands and the full accounting for all 59 crates are in `evidence.md` §MS-1. A new `unsafe` in a forbid crate fails to compile; all other sites are ratcheted by MS-3. |
| MS-2 | **Every first-party `unsafe` site is enumerated + justified** — the 87-site register records each site's kind, the invariant it relies on, and how it is bounded/tested. | **PASS** | `compliance/memsafety/unsafe-register.md` (87 rows). Count cross-checked: `scripts/unsafe-gate.py --list` → `TOTAL=87`, matching `bench/unsafe-snapshot.json` (`total: 87`) and the per-crate table in `evidence.md` §MS-2. |
| MS-3 | **Unsafe-count RATCHET gates every PR** — a PR cannot add an `unsafe` site without updating the register + re-seeding the snapshot; the count gate is merge-blocking. | **PASS** | `scripts/unsafe-gate.py --check` (re-run: PASS, live 87 == snapshot 87). CI job `unsafe-register (count ratchet)` in `.github/workflows/ci.yml` (`unsafe-register:` job, no `continue-on-error`, name has no "informational"/"advisory" token ⇒ the `ci-summary / gate` aggregator treats it as REQUIRED). |
