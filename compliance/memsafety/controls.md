...
|---|---|---|---|
| MS-1 | **Unsafe surface is *confined*** — 50 first-party crates carry a source `forbid(unsafe_code)` and 8 are unsafe-bearing (the sets overlap for conditional/test-only cases). | **PASS** | Re-runnable per-crate-deduped commands and the full accounting for all 60 crates are in `evidence.md` §MS-1. A new `unsafe` in a forbid crate fails to compile; all other sites are ratcheted by MS-3. |
