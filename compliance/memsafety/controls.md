...
|---|---|---|---|
| MS-1 | **Unsafe surface is *confined*** — only 5 first-party crates contain `unsafe`; the other 26 are `#![forbid(unsafe_code)]`, so a new `unsafe` anywhere else fails to compile. | **PASS** | `#![forbid(unsafe_code)]` in 26 crates (verified: `grep -rl forbid(unsafe_code) crates/` → 26 crates; 31 crates total). Enumerated in `evidence.md` §MS-1. |
