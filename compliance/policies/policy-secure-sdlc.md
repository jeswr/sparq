...
| **Static analysis (SAST)** | **In force:** clippy `--all-targets -- -D warnings` — a hard merge gate; plus the unsafe-count ratchet, cargo-deny/cargo-vet and the fuzz/Miri lanes. **Suspended, and NOT currently policy:** CodeQL `security-and-quality`. The lane has been **disabled at the Actions level (`disabled_manually`) since 2026-07-18** by maintainer direction (merge latency) — it runs on no event and gates nothing — and **35 critical `rust/hard-coded-cryptographic-value` alerts are open**, triaged as false positives of one query-model defect (#4615). The former "code-scanning alerts kept at zero" rule is therefore **withdrawn as a statement of fact**: it was false on both counts. **Operative policy today: this project runs NO SAST.** No compensating control performs taint or crypto-misuse analysis; clippy is a lint, not a security analyser. Re-instating a SAST requirement (re-enable advisory-only / on a schedule / adopt another tool / formally accept no SAST) is a **pending maintainer decision — issue #4620**; until it is taken, PW.7 is met only by human review + the lint/dynamic lanes. | `.github/workflows/ci.yml#clippy` (in force); `.github/workflows/codeql.yml` (present but disabled); gap **GX-14** in `../gap-register.md`; `ASSURANCE.md` §11 | PO.2.1, PW.7 |
