...
|---|---|---|---|---|---|---|
| **ASVS-G1** | `sparq-server` sets **no security response headers** (`X-Content-Type-Options: nosniff` at minimum; no CSP/`X-Frame-Options`/HSTS). | V14.4 | **Medium** | Add a `tower` layer setting `X-Content-Type-Options: nosniff` on all responses (and `Cache-Control: no-store` on auth/error responses). Add a regression test asserting the header. Residual risk is **low** (JSON API, no HTML rendering sink, no ambient cookie auth) but `nosniff` is cheap and expected at L2. | next server PR | `sq-2bhm` |
| **ASVS-G2** | **No first-party CORS allowlist option.** sparq-server emits no CORS headers, so cross-origin browser reads are denied by same-origin policy. | V14.5.3 | **Low** (documented decision) | This is a **safe default**, not a vulnerability — cross-origin access is the operator gateway's job (B3). *Optional* nice-to-have: an opt-in `--cors-allow-origin <allowlist>` that never defaults to `*`. Not required for L2 sign-off. | optional | `sq-vtkj` |
| **ASVS-G3** | Engine **parse/exec error strings are echoed to the client** (query-author UX) with no test asserting they never contain a filesystem path / internal host / resolved addr. | V7.1.1 | **Medium** | Audit `engine_error_response` / `execution_error` message sources; add a regression test that error bodies for a crafted query/file contain no absolute-path prefix or resolved `host:port`. (The structured `json_error` envelope + panic isolation already pass V7.1.2/V7.4.) | next server PR | `sq-kfel` |
| **ASVS-G4** | **No explicit SPARQL parse-depth bound** — deeply-nested-input DoS is bounded only by `--max-body-bytes`, not a depth constant (SHACL has explicit guards; the SPARQL path relies on `spargebra` recursive descent). | V5.5.2 | **Low** | Add a deeply-nested-SPARQL corpus entry to the existing `parse_sparql` fuzz target asserting no stack overflow at the 1 MiB body cap, **or** add an explicit nesting-depth guard. The body cap already bounds total input; this closes the *recursion-depth* sub-case. | fuzz corpus update | `sq-1ukn` |
