...
| CD-2 | **No per-query access audit trail.** Only aggregate Prometheus metrics; no identity/graphs-touched/decision log. | 3.2 (2→3) | P0 | Opt-in structured JSON-lines access log emitting the WAC session identity + authorised graph set sparq already computes. | `CDMC: opt-in structured access audit log (identity + graphs touched)` |
