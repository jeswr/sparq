...
| **Branch-Protection** | AR | [`docs/branch-protection.md`](../../../docs/branch-protection.md) (doc-of-record) — required `ci-summary / gate`, ≥1 CODEOWNERS review, dismiss-stale, linear history, block force-push, no-admin-bypass. | maintainer | The *settings* live in GitHub UI (out-of-repo); the doc records the intended ruleset. Scorecard reads the live ruleset via API — score reflects whatever is actually configured. **AR** because verification needs the live API/dashboard. |
...
| **CII-Best-Practices** | GAP | Badge not yet filed on bestpractices.dev (GX-4). Drafted answers ready in [`evidence.md`](../evidence.md). | maintainer | Scorecard reads the badge's published level; 0 until the badge entry exists. Bead **sq-toze.5**. |
| **Code-Review** | AR | [`docs/branch-protection.md`](../../../docs/branch-protection.md) (≥1 approving review + CODEOWNERS + Copilot auto-review); [`CODEOWNERS`](../../../CODEOWNERS); [`.github/PULL_REQUEST_TEMPLATE.md`](../../../.github/PULL_REQUEST_TEMPLATE.md). | maintainer | Scorecard infers review from merged-PR history. A solo-maintainer repo can score below 10 here even with the rule enabled (self-review is discounted); honest nuance, not a hideable gap — see gap-register GX-OSSF-3. |
...
| **Maintained** | AR | Active commit/PR cadence on `main`; `SECURITY.md` declares maintenance posture. | maintainer | Scorecard reads the last-90-days commit/issue activity at scan time. |
...
| **Vulnerabilities** | IV | [`.github/workflows/supply-chain.yml`](../../../.github/workflows/supply-chain.yml) (`cargo deny check bans sources licenses`, gating) + the daily advisory watchdog [`dependency-monitoring.yml`](../../../.github/workflows/dependency-monitoring.yml). PR-time *advisory* gating is degraded (cargo-deny CVSS-4.0 parse bug, GX-1 / bead sq-q8de) — the daily watchdog is the real advisory gate; stated honestly. | maintainer | Scorecard checks for *open, unfixed* OSV vulns in the repo at scan time. |
