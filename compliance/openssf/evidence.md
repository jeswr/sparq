...
- **`warnings_strict`** — Met — `cargo clippy --workspace --all-targets -- -D warnings` (full-workspace, all-targets) is the **hard gate** ([`ci.yml`](../../.github/workflows/ci.yml) `clippy + fmt (gate)` job). *(This is the gold-level strict-warnings posture; the criterion rests on the clippy hard-gate. Since sq-0lk the same job also hard-gates `cargo fmt --all --check` — the one-time workspace reformat has landed — but formatting is style, not a warning class, so it is noted here rather than cited as enforcing the criterion.)*
