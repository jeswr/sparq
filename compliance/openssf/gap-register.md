...
| **GX-OSSF-4** | **The CodeQL SAST lane is operationally disabled — the repository has no security-query static analysis of its own Rust.** `.github/workflows/codeql.yml` is retained on `main` with live triggers, but the workflow's Actions state is `disabled_manually` (set out-of-band 2026-07-18), so GitHub schedules no run on any trigger: no `CodeQL analysis (rust)` check-run, no CodeQL code-scanning alert, and the ruleset's `code_scanning` rule exerts no pressure. This is **not fixable by a commit** — a `disabled_manually` workflow stays disabled until a maintainer flips it in the Actions UI or via `gh workflow enable`; editing or merging changes to the file does nothing (which is why open PR #3427, "re-enable as advisory", can deliver only its advisory-registry half). Scorecard's `SAST` check loses its CodeQL signal, and Badge `static_analysis_common_vulnerabilities` is answered **Unmet** in the self-cert as a result. | P1 | **OPEN — state recorded; the flip is maintainer-owned and external to the repo** | **Recorded, not remediated (issue #5367).** The durable posture, the exact re-enable procedure, and the two consequences of re-enabling (the check GATES on day one unless an `.github/advisory-registry.json` entry lands first or with the flip; an alert backlog arrives at once against the standing zero-open-alerts rule) are in [`docs/branch-protection.md` §CodeQL SAST — the durable posture](../../docs/branch-protection.md#codeql-sast--the-durable-posture-issue-5367). **Compensating controls (all verifiable on `main`, none a substitute for the security query suite):** clippy `-D warnings` gates every PR ([`ci.yml`](../../.github/workflows/ci.yml)) — a lint, not a vulnerability scanner; [`miri.yml`](../../.github/workflows/miri.yml) / [`asan.yml`](../../.github/workflows/asan.yml) / [`kani.yml`](../../.github/workflows/kani.yml) / [`fuzz.yml`](../../.github/workflows/fuzz.yml) for UB, memory safety and bounded proofs on the crates they reach; gating `cargo deny` plus the daily [`dependency-monitoring.yml`](../../.github/workflows/dependency-monitoring.yml) for *dependency* advisories; Trivy SARIF ([`container-scan.yml`](../../.github/workflows/container-scan.yml), [`release.yml`](../../.github/workflows/release.yml)) as the only current code-scanning feeder; Copilot review on push. **Residual risk, stated plainly:** taint/injection-class findings in first-party Rust would today be caught by nobody, and the disabled state raises no alarm of its own (the detector that would notice the missing weekly cron, [`ci_execution_latency_alarm.py`](../../scripts/ci_execution_latency_alarm.py) M1, counts a non-`active` workflow in its census and skips it by design) — this row and the branch-protection section are the standing record. **Exit:** the maintainer runs `gh workflow enable codeql.yml`, verifies `gh api repos/jeswr/sparq/actions/workflows/codeql.yml --jq .state` reports `active`, and closes this row; or decides the disable is permanent, in which case `codeql.yml` and its Scorecard/Badge claims should be removed rather than left dormant. | maintainer to flip or to declare permanent | **issue #5367** (no bead — the action is external to the repo) |
...
The in-repo OpenSSF posture is **strong**, with one real operational hole. Every
repository-level Scorecard check that can be asserted from the codebase/CI is
**implemented & verified**, and the Best-Practices badge is **answer-ready at the passing
level** (with silver/gold reach on warnings, signed releases, and fuzz+Miri analysis). Two
gaps are **external human-owned steps** the agent cannot perform: the badge filing (GX-4)
— still the only blocker to a *displayed* badge — and the CodeQL flip (**GX-OSSF-4**).
GX-OSSF-4 is not a maturity nudge: the SAST lane is **off right now**, so first-party
security-query analysis is genuinely absent and the compensating controls listed in that
row do not replace it. The two remaining gaps (GX-OSSF-2 registry-publish signing,
GX-OSSF-3 solo-maintainer review score) are maturity nudges, not posture failures. No
control is overclaimed — the self-cert's static-analysis answers were reconciled against
GX-OSSF-4 rather than left citing a workflow that does not run; the advisory
