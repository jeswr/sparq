...
| **CIS** | [`cis/`](./cis/) | CIS v8 safeguards + Docker Benchmark §4 image hardening | **PASS / N/A(operator)** except GX-12 (P1) + GX-13 (P3) | Container CVE scan + Dockerfile linter (`sq-toze.31`) |
| **SBOM + supply-chain** | [`sbom/`](./sbom/) | NTIA elements, CycloneDX, VEX, signed/attested per-release SBOM | **Strong** — 22 verified / 6 audit-ready / 3 gap (all P2) | Publication/signing operating-verified on first `v*` release |
| **NIST SSDF** | [`ssdf/`](./ssdf/) | SP 800-218 PO/PS/PW/RV → sparq control mapping | **28 verified / 13 audit-ready / 1 gap** | Reproducible-build evidence (GX-8) |
| **SLSA** | [`slsa/`](./slsa/) | Build provenance / supply-chain integrity of released artifacts | **Build Level 2** (honestly claimed; **not L3**) | L3 in-band-provenance ceiling; unattested dist/published packages |
...
| **ISO/IEC 27001** | [`iso27001/`](./iso27001/) | Annex A control mapping + **org-adoptable ISMS template set** (clauses 4–10 + full SoA), not a certificate | **Zero open Annex-A control gaps**; GAP-ISO-1 templates **delivered**, GAP-ISO-2 open | ISMS/SoA + certificate **org act** (GAP-ISO-1 residual, external) |
