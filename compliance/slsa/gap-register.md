...
| **GX-11** | **Build L3 not met — NARROWED (sq-toze.25), still OPEN.** The **release archives** now get their provenance from an isolated trusted builder: `release.yml#provenance` calls `slsa-framework/slsa-github-generator/.github/workflows/generator_generic_slsa3.yml@v2.1.0` in a **separate job** that runs no step of ours (a bare `uses:`), mints its own OIDC identity, and receives exactly one thing across the boundary — `base64-subjects`, the archive digests threaded from `build-matrix.yml`'s new `hashes` output. The digest-collecting job builds nothing and holds no signing identity; the builder job has `id-token: write` but **not** `contents: write` (`upload-assets: false`). `release` **`needs:`** it, so no Release is cut without it, and the signed `sparq-cli-<version>.intoto.jsonl` is attached + covered by `SHA256SUMS`. **Two residuals keep it open:** (a) **unexercised** — `release.yml` is tag/dispatch-triggered, so the lane first runs on the next `v*` tag; wiring is not evidence, and the published level stays L2 until a tag proves it. (b) **partial coverage** — the ghcr image, GUI bundles, SBOM/VEX, conformance report and `dist.yml` binaries still attest **in-band** (L2); (c) it is the **generic** generator, which signs digests our `package` job reported rather than building the artifact itself — so it delivers unforgeable *provenance*, not a hardened build. Unglamorous but honest: a compromised build step now yields truthful provenance about a compromised archive. | P3 | SL-B3-b | (a) Cut a `v*` tag; confirm `slsa-verifier verify-artifact <archive> --provenance-path sparq-cli-<version>.intoto.jsonl --source-uri github.com/jeswr/sparq` passes, then flip SL-B3-b to **IV** for the archives. (b) Extend to the container image (`generator_container_slsa3.yml`) and the remaining artifacts. The workflow shape is regression-pinned on every PR by `scripts/tests/test_release_slsa_l3_provenance.py` (stdlib, mutation-proved) — `release.yml` never runs on a PR, so without it a silent revert to in-band attestation would be invisible. | archives WIRED (unverified); rest aspirational | **sq-toze.25** |
...
- Close **GX-11** → **Build L3** (isolated, non-falsifiable provenance) — the headline level rises.
  The mechanism now exists for the archives; what remains is *evidence* (one `v*` tag that runs the
  trusted-builder job and yields a `.intoto.jsonl` that verifies) plus extending it beyond the
  archives. Until both, the headline stays L2 — see the GX-11 row.
