...
| **Gap** | published-package provenance PARTIAL (GX-10/sq-toze.24 + sq-toze.37): **npm CLOSED** (`publish.yml#npm`), **crates.io** has an out-of-band `.crate` attestation but the **registry-native link is external/OPEN**, **PyPI `sparq-rdf`** PEP-740 lane WIRED in CI (`publish.yml#pypi-*`, Trusted Publishing + native attestations) but awaits a one-time maintainer PyPI Trusted-Publisher registration; reproducible-build CHARACTERISED not enforced (GX-8/sq-toze.9 — `reproducible-build.md`: 22-byte single-cause diff documented, CI rebuild-and-diff ratchet remaining); Build L3 not met — NARROWED (GX-11/sq-toze.25): the **release archives** now route provenance through the isolated `slsa-github-generator` trusted builder (`release.yml#provenance`, separate job, digests threaded across the boundary, `release` `needs:` it), but the lane is **unexercised** (tag-triggered — first runs on the next `v*` tag) and every other artifact (container, GUI bundles, SBOM/VEX, conformance report, `dist.yml` binaries) is still in-band L2, so the published level stays **L2**. *(GX-9 dist.yml binaries — CLOSED, now SLSA Build L2 / sq-toze.23.)* |
