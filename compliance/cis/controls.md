...
| C-16.12 | **16.12 Implement code-level security checks (SAST).** | **PASS** | CodeQL (`codeql.yml`, runs on every eligible non-draft Rust-changing PR plus every merge_group/push/schedule, results to code-scanning; advisory at merge since 2026-07-17 — alerts triaged retroactively via the daily `codeql-alert-sweep.yml` issue); clippy `-D warnings` (`ci.yml#lint`, gating). | sparq |
