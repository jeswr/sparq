...
| **CR-14** | **Constant-time / side-channel posture is documented** — where timing leakage matters and where it is accepted. | **AUDIT-READY** (documented; no externally-verified CT proof) | [`evidence.md`](./evidence.md) §CR-14. The Schnorr *verify* path (`sparq-zk/src/sig.rs`) is verifier-side over public commitments (no long-term secret in the timing-sensitive path). The MPC mask draw is documented effectively-constant-time-in-expectation (`sparq-mpc/src/rng.rs:45`). **No formal CT analysis / `dudect`-style measurement has been done** — recorded as CR-G5. The arkworks BN254 field ops are not asserted constant-time by sparq. |
| **CR-15** | **FIPS posture is stated honestly** — sparq makes NO FIPS claim and uses no validated module. | **PASS (honest negative claim)** | [`evidence.md`](./evidence.md) §CR-15. BN254 / Poseidon2 / Schnorr-over-Baby-JubJub are **not** FIPS-approved algorithms and not intended to be (they are ZK-friendly primitives). SHA-256 (release digests) is FIPS 180-4 but is used for integrity, not as a validated crypto module. No CMVP certificate is claimed (CR-G4). |
