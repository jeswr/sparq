...
| **CR-G5** | No instrumented constant-time / side-channel measurement; a **source-level** review exists (`side-channel-analysis.md`). Present exposure is argued LOW by *architectural placement* (verify path carries no secret; signing is issuance-side), **not** by constant-time primitives. The auditor must judge whether an instrumented `dudect`/`ctgrind` pass is required for the threat model. | P2 | ADDRESSED-by-analysis (`sq-egx6`; hardening beads `sq-u8a8`/`sq-19ej`/`sq-it50`/`sq-7ltf`/`sq-8jv7` landed) |
