...
| **GX-14** | **SAST is NOT running — CodeQL is operationally disabled, and nothing compensates.** [OPUS-5] `.github/workflows/codeql.yml` has been disabled at the Actions level (`disabled_manually`) since **2026-07-18** by separate maintainer direction (merge latency). The file and its triggers are retained on `main`, but GitHub schedules no run on any event: **no** `CodeQL analysis (rust)` check-run, **no** SARIF upload, and it gates nothing — including on `merge_group`, so the ruleset's `code_scanning` rule has no live feeder from CodeQL. **This invalidates CodeQL as evidence** wherever a slice cites it as an implemented/verified/PASS control (SSDF PO.3.1/PO.3.2/PW.7.1/PW.7.2/RV.3.4, OpenSSF `SAST` + `static_analysis*`, CIS C-16.12 + C-7.5/7.6, ISO A.5.7/A.5.8/A.8.7/A.8.8/A.8.25/A.8.28, ASVS V1.1/V10.2, CRA I.1/II.3, the two `policies/` SAST rows, CDMC 4.1); each is now qualified in place. **No compensating SAST control exists** — clippy `-D warnings`, the unsafe-count ratchet, `cargo-deny`/`cargo-vet` and the fuzz/Miri lanes are all live and genuine, but none performs taint or crypto-misuse analysis, so the *residual* is real, not bookkeeping. The 35 open critical alerts CodeQL left behind were **triaged** under issue #4615 and found to be false positives of a single query-model defect (`ASSURANCE.md` §11) — triaged is not covered, and says nothing about what an enabled scanner would find now. Closing this needs a maintainer decision on the durable posture (re-enable advisory-only / re-enable on a schedule / adopt another SAST / accept and document no SAST). | **P1** | ssdf, openssf, cis, iso27001, asvs, cra, cdmc, policies | **#4620** (posture decision) · #4615 (doc reconciliation) | **OPEN** |
