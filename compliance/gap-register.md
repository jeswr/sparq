...
| **SLSA Build L3** | Build **L2 is met and honestly claimed**; **L3 is NOT**. The trusted-builder migration has **started** — the release archives now sign via the isolated `slsa-github-generator` job (sq-toze.25) — but it is unexercised (tag-triggered) and does not yet cover the container/GUI/SBOM/dist artifacts, so nothing L3 is claimed. The certificate-grade L3 audit remains external regardless. | slsa | finish the trusted-builder migration (container + remaining artifacts) + one `v*` tag as evidence + accredited assessor | **`sq-toze.25` (P3, GX-11)** |
...
| **GX-11** | **SLSA Build L3 not met — NARROWED (sq-toze.25).** The **release archives** now route provenance through the isolated `slsa-github-generator` trusted builder (`release.yml#provenance` — separate job, bare `uses:`, own OIDC identity, digests threaded via `build-matrix.yml`'s `hashes` output, `release` `needs:` it, signed `.intoto.jsonl` attached + in `SHA256SUMS`). **Still OPEN on two counts:** the lane is **unexercised** (`release.yml` is tag/dispatch-triggered — first runs on the next `v*` tag, and wiring is not evidence), the container image, GUI bundles, SBOM/VEX, conformance report and `dist.yml` binaries remain **in-band L2**, and the *generic* generator signs digests our build job reported rather than building inside the trusted builder (unforgeable provenance, not a hardened build). Published level stays **L2**. | P3 | slsa | **`sq-toze.25`** | OPEN (archives wired-unverified; rest aspirational) |
...
### SLSA — Build L2 met; GX-9/GX-10/GX-8/GX-11 (all in cross-cutting table). GX-11 narrowed by sq-toze.25 (archives on an isolated trusted builder — wired, not yet exercised).
