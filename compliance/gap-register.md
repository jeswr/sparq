...
| **GX-11** | **SLSA Build L3 not met — NARROWED TWICE (sq-toze.25, then #4570).** Every artifact the `release.yml`/`dist.yml` pipelines publish, except the container image, now routes provenance through the isolated `slsa-github-generator` trusted builder — three separate jobs, each a bare `uses:` with its own OIDC identity and only a threaded digest list crossing the boundary: `release.yml#provenance` (archives), `release.yml#provenance-artifacts` (GUI bundles, SBOM/VEX, conformance report), `dist.yml#provenance` (tiered binaries). `release` `needs:` both of its lanes; both signed `.intoto.jsonl` bundles are attached + in `SHA256SUMS`. **Still OPEN on three counts:** no lane is **exercised** (`release.yml` is tag/dispatch-triggered, `dist.yml` dispatch-only — wiring is not evidence), the **ghcr container image** remains **in-band L2** (it needs `generator_container_slsa3.yml`, a distinct migration), and the *generic* generator signs digests our build jobs reported rather than building inside the trusted builder (unforgeable provenance, not a hardened build). Published level stays **L2**. | P3 | slsa | **`sq-toze.25`** + **#4570** | OPEN (all file artifacts wired-unverified; container aspirational) |
...
### SLSA — Build L2 met; GX-9/GX-10/GX-8/GX-11 (all in cross-cutting table). GX-11 narrowed by sq-toze.25 (archives) then #4570 (GUI bundles, SBOM/VEX, conformance report, dist binaries) onto isolated trusted builders — all wired, none yet exercised; the ghcr image is the remaining in-band artifact.
