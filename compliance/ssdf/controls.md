...
| **PO.3.1** | Specify which tools (or tool types) are mandated/recommended for each stage. | Implemented & verified | `deny.toml` (cargo-deny policy) + `.github/workflows/supply-chain.yml` (cargo-deny / cargo-vet / cargo-cyclonedx) + `codeql.yml` (CodeQL SAST) + `miri.yml` + `fuzz.yml` + `ci.yml` clippy/fmt. The mandated tool set is encoded as CI jobs (gating, except CodeQL — advisory at merge since 2026-07-17 with retroactive alert triage — and the scheduled Miri/nightly-fuzz lanes). | @jeswr |
| **PO.3.2** | Configure tools to improve assurance & enforce; integrate into the SDLC. | Implemented & verified | clippy `-D warnings` (gate, `ci.yml#clippy`), CodeQL `security-and-quality` queries (`codeql.yml`, advisory at merge), cargo-deny `bans sources licenses` **and** `advisories` gating (`supply-chain.yml#audit`), cargo-vet gating (`supply-chain.yml#vet`), the unsafe-count ratchet (`ci.yml#unsafe-register`, `scripts/unsafe-gate.py`). | @jeswr |
...
| **PW.7.1** | Determine whether code review and/or analysis should be used; configure it. | Implemented & verified | PR-only flow with required `@jeswr` review on CODEOWNERS paths (`docs/branch-protection.md`); SAST = CodeQL `security-and-quality` (`codeql.yml` — advisory at merge since 2026-07-17; alerts triaged retroactively via the daily `codeql-alert-sweep.yml` issue). Both human review and automated analysis are mandated. | @jeswr |
| **PW.7.2** | Perform the code review / static analysis per org policy. | Implemented & verified | clippy `-D warnings` (`ci.yml#clippy`, merge-blocking via `ci-summary / gate`); CodeQL is wired to push/PR/merge_group + schedule (`codeql.yml`) and is advisory at merge (2026-07-17), with code-scanning alerts triaged retroactively to zero via the daily `codeql-alert-sweep.yml` rolling issue (fix, or dismiss with a written reason). **CodeQL currently produces no analysis** — the workflow is Actions-disabled (see `docs/branch-protection.md`, OPERATIONALLY DISABLED note); clippy is the live gating static analysis until it is re-enabled. | @jeswr |
...
| **RV.3.4** | Review the SDLC to detect classes of the vulnerability proactively (e.g. add a check). | Implemented & verified | New checks added in direct response to risk classes: the fuzz lane (parse/mmap crash classes), the Miri lane (UB classes on the unsafe surface), the unsafe-count ratchet (unsafe-creep class, gating), CodeQL (SAST class; advisory at merge since 2026-07-17 with retroactive triage). Each is a proactive class-detection control. | @jeswr |
