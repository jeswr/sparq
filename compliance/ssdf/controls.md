...
| **PW.7.2** | Perform the code review / static analysis per org policy. | Implemented & verified | CodeQL runs on push/PR/merge_group + schedule (`codeql.yml`); clippy `-D warnings` (`ci.yml#clippy`, merge-blocking via `ci-summary / gate`). CodeQL is advisory at merge (2026-07-17); its code-scanning alerts are triaged retroactively to zero via the daily `codeql-alert-sweep.yml` rolling issue (fix, or dismiss with a written reason). | @jeswr |
