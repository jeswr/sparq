...
| GX-CRA-2 | **No Article 14 ENISA/CSIRT reporting runbook.** From 2026-09-11, actively-exploited vulnerabilities + severe incidents must be reported to ENISA/the CSIRT (early-warning 24h / notification 72h / final report) via the single reporting platform. sparq has coordinated disclosure but no documented escalation runbook to the authority. | Art. 14; controls.md CRA-CA.5 | P1 | Author a reporting-runbook **policy template** under `compliance/policies/` and reference it from `SECURITY.md`. The *act* of reporting is an organizational/legal duty (out of agent scope); the runbook is in scope. | `sq-iy3p` |
