...
| The tolerated advisory is justified + VEX'd | `deny.toml [advisories].ignore` ↔ `supply-chain/vex.cdx.json` | Currently **zero** tolerated advisories — the ignore list and the VEX `vulnerabilities[]` are both empty (still 1:1). Both former entries were retired: rustls-pemfile/RUSTSEC-2025-0134 → sq-g2xs (migrated ureq 2 → ureq 3, whose rustls-native-certs 0.8 dropped the archived crate, so it left the tree); paste/RUSTSEC-2024-0436 → sq-l8bv (the GPU stack no longer pulls `paste`). When a future `unmaintained` informational advisory needs tolerating, add the ignore AND a matching VEX `not_affected` entry together. |
