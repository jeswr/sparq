...
| VEX-2 | VEX states exploitability for **every** ignored advisory | **Implemented & verified** | VEX covers `RUSTSEC-2025-0134` with `analysis.state=not_affected` + a CycloneDX `justification` (`vulnerable_code_not_in_execute_path`) + detail. (`RUSTSEC-2024-0436` paste was dropped from both deny.toml and the VEX once the GPU stack stopped pulling `paste`.) | SPARQ |
