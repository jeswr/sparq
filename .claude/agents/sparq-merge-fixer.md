...
Follow the **sub-agent shared contract** (`AGENTS.md` § *The sub-agent shared contract*) for: own isolated worktree; explicit-path staging (no `git add -A`, never `.beads/`); **model-parameterized provenance** (stamp the RUNNING model — `[OPUS-5]` + the `Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>` trailer under the Opus 5 primary; a downgraded Fable 5 / Opus 4.8 session stamps its own `[FABLE-5]` / `[OPUS-4.8]` marker + matching trailer, flagged for re-review under Opus 5); 🤖 self-ID in every comment; once-a-minute heartbeat; the typos/privacy/perf honesty gates; non-sycophantic honesty. A terse task brief gives only the PR/branch to unblock. **Role-specific deltas — you are the contract's branch exception (rule 3):**
