...
- **Self-restrictions — CR-Self** — `owl:hasSelf "true"^^xsd:boolean` + `owl:onProperty r` is the EL
  profile's `ObjectHasSelf` (`∃r.Self`, local reflexivity): `X ⊑ ∃r.Self ⇒ (X,X) ∈ R(r)`, `∃r.Self ⊑ D`
  threads via the self-concept atom + CR1, and a SAME-NOMINAL self-link `({a},{a}) ∈ R(r)` (an
  asserted/derived `a r a`) reads off as `∃r.Self ∈ S({a})` (CRs3 — sound: a nominal is a singleton).
  With `rbox` it propagates to `∃s.Self` for `r ⊑* s`; a general `(X,X)` link from `X ⊑ ∃r.X` never
  triggers either (load-bearing), malformed shapes stay counted skips, `abox` self-loops realise as `a r a` (WG -001/-002).
