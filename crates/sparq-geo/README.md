...
| 8.4 (Req 9) | Geometry-**metadata** properties: `geo:dimension`, `geo:coordinateDimension`, `geo:spatialDimension`, `geo:isEmpty`, `geo:isSimple`, `geo:hasSerialization` | ✅ materialised by `GeoGeometry::metadata` / the `metadata::lex` mirror (sq-mzmh): topological dimension (max over a collection), coordinate/spatial dimension = 2 in the 2-D SF profile, emptiness, self-intersection-free simplicity, and the WKT serialization. [OPUS-4.8] |
| 8.5.2 | `geo:gmlLiteral` | ✅ GML Simple-Features geometry profile (`parse_gml_literal`): `gml:Point`, `gml:LineString`, `gml:Polygon` (exterior + interior rings), `gml:MultiPoint`/`MultiCurve`/`MultiSurface` (+ the GML 2 `MultiLineString`/`MultiPolygon` aggregates) plus `gml:MultiGeometry`/`GeometryCollection`; an empty lexical form OR a member-less aggregate is the empty geometry (Req 16); `srsName`→CRS (incl. URN/`EPSG:` spellings, EPSG:4326 axis swap) identical to the WKT path; `geof:` functions and `GeoIndex` (`geo:asGML`) treat it interchangeably with WKT. `gml:Envelope`, arc-segment `gml:Curve`/`Surface`, and 3-D coordinates are deferred (clean `GeoError::Unsupported`; `bd list -l area:sparq-geo`) |
...
demonstrates (currently **30 / 30** after sq-mzmh closed the last two gaps — R9
geometry-metadata properties and R16 empty-`gmlLiteral`). The table above is
