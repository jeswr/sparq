...
**Streamable HTTP** *(feature `http`, OFF by default)* is the remote/multiplexed transport: `serve_http(listener, transport)` serves many concurrent clients on one endpoint (`/mcp`), each with an `Mcp-Session-Id` and its own SSE stream (`GET`, with `Last-Event-ID` resumption; `HttpTransport::notify`/`broadcast` push to it), over one shared `Mutex<McpServer>` — so the transport is concurrent but engine access is serialised. Std-only framing, **zero** added crates. `Origin` is validated (by default *any* `Origin` is refused), and both sessions and concurrent connections are capped (an over-cap socket is answered `503` without spawning a worker — the bound on a peer that connects and never initializes), but that is DNS-rebinding + DoS hedging, **not authentication** — bind loopback. See `examples/mcp_http_server.rs`. <!-- [SONNET-4.6] sq-2c0f0 -->
...
writes route through the pod store's atomic fail-closed ACL write-through. RDF sources
only (Turtle / N-Triples) in v1. Its own `resources` surface adds **`subscribe: true`**:
