...
impl Graph {
    ...
    /// (also "json-ld" / "application/ld+json"). JSON-LD `@graph` named graphs are
    /// folded into the default graph here; use [`load_dataset`](Self::load_dataset) to
    /// preserve them.
...
/// `load_dataset`) recognises the same set.
