...
mod byte_attribution {
    ...
    fn store_bpt_at(triples: &[[Id; 3]], bs: usize) -> (f64, f64) {
        ...
        (
            (agg.total() + dir) as f64 / n,
            block_overhead(&agg, dir) as f64 / n,
        )
    }
    ...
    fn block_size_cannot_flatten_the_scale_growth() {
        ...
        assert_eq!(
            sparse.len(),
            dense.len(),
            "regimes must be equal-sized to compare B/triple"
        );
...
mod tests {
    ...
    fn bloom_probe_sets(rows: &[[Id; 3]], cap: usize) -> (Vec<Id>, Vec<Id>) {
        ...
        let sample: Vec<Id> = present
            .iter()
            .copied()
            .step_by(present.len() / cap + 1)
            .collect();
    ...
    fn bloom_model_matches_shipped_filter() {
        ...
        assert!(
            chunks.len() > 4,
            "need several blocks to compare per-block filters"
        );
        ...
        assert_eq!(
            shipped.heap_bytes(),
            model.heap_bytes(),
            "model/shipped filter size differ"
        );
    ...
    fn measure_bloom_skip_and_sizing() {
        ...
        for (name, order, scatter) in columns {
            ...
            for (label, probes) in [("present", &sample), ("absent", &absent)] {
                let (mut cand, mut skipped, mut fp, mut truly_absent_blocks) =
                ...
                for hashes in [1u32, 2, 3, 4] {
                    ...
                    println!(
                        ...
                        if n_absent == 0 {
                            ...
                            100.0 * fp as f64 / n_absent as f64
                        },
                        100.0 * theory
                    );
                }
    ...
    fn measure_bloom_selective_scan_latency() {
        ...
        for (name, order, scatter) in columns {
            ...
            if !c.has_bloom() {
                println!(
                    "--- column {}: density gate declined; no ON arm to time ---",
                    name
                );
                continue;
            }
            ...
            for (label, probes) in [("present", &hit), ("absent", &miss)] {
                ...
                assert!(
                    blocks_off > 0,
                    "{} arm visited no candidate blocks — vacuous",
                    label
                );
                assert_eq!(
                    sum_on1, sum_off1,
                    "{} arms disagree on rows returned",
                    label
                );
                assert_eq!(
                    (sum_on1, sum_off1),
                ...
                assert!(
                    label != "present" || sum_on1 > 0,
                    "present probes returned no rows — vacuous"
                );

                let rounds = 2;
                let per = |d: std::time::Duration| {
                    d.as_nanos() as f64 / (probes.len().max(1) * rounds) as f64
                };
                println!(
