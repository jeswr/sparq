...
4. **Parse/plan once** — DEFERRED, **blocked-on-engine-seam**: `sparq_engine`
   only exposes the SPARQL-string API; the engine-seams work is adding a
   pre-parsed-algebra entry point. When it lands, cache the `spargebra`
   algebra at `register` time and evaluate it directly (worth ~µs/window —
   only visible at very high window rates, the RANGE 100 line).
