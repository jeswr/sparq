...
- Design records: [`research/lws-design-records.md`](../../research/lws-design-records.md) — the in-repo home for this crate's migrated `decisions/` + `docs/design/` estate (sq-gg0qq.10), reconstructed from the code. Doc-comments here still cite source-repo paths; §1 of that record maps each one. `bench/` stays in the source repo.
- Normative specs — the spec is the contract, not this implementation: [DPoP-SK](https://jeswr.github.io/dpop-sk-spec/) for `src/pop/sk/`, and the pinned [solid-oidc-verifier](https://github.com/jeswr/solid-oidc-verifier) for token/DPoP verification. Neither is reimplemented here.
