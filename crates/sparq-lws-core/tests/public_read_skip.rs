...
//! and pin the security invariants of `research/lws-design-records.md` §5 (RSS `decisions/0002`).
//! The HARD SCOPE LIMIT the WAC-Allow conformance suite enforces is load-bearing: the skip fires
//! ONLY for a GET/HEAD that carries NEITHER an `Authorization` NOR a `DPoP` header; a CREDENTIALED
//! read is never short-circuited (so an authenticated owner of a public resource sees their full
//! `WAC-Allow user=` modes, and a forged token is rejected, not served). A malformed `DPoP` header
//! (even without `Authorization`) keeps the auth path's canonical 400.
//!
