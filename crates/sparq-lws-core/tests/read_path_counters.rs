...
//! read-1 (RSS `docs/design/backend-read-path.md` §7 → `research/lws-design-records.md` §7):
//! PINNED deterministic backend round-trip counts per read operation, measured end-to-end through
//! the assembled router (auth → WAC → LDP → store) with the counting decorators at the
