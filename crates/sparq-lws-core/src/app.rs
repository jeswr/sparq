...
/// RSS `docs/design/webid-outside-pod.md` → `research/lws-design-records.md` §4).
    ...
    S: Store + 'static,
{
    ...
    let protected = Router::new()
        ...
        // PRE-CRYPTO PUBLIC-READ SKIP (skip-crypto opt 3, RSS `decisions/0002` →
        // `research/lws-design-records.md` §5): a cheap, identity-independent fast-path BEFORE
        // crypto, in the SAME slot as the rate-limit / overload layers — just INSIDE CORS, just
        // OUTSIDE auth. For a GET/HEAD that carries NO `Authorization`/`DPoP` header it constructs
        // a public token and delegates STRAIGHT to the same `serve_read` the handler uses (one
        // anonymous WAC pass): a PUBLIC read → 200, an anonymous denial → the same 401 + challenge,
        // a malformed target → the canonical 400 — all byte-identical to the full anonymous path.
        // A CREDENTIALED request (Authorization OR DPoP header present), a mutation, or any other
        // verb FALLS THROUGH (`next.run`) to the unchanged auth path. It carries the LDP state
        // (store + base + ACL cache) so the served read uses the SAME `serve_read`.
