...
/// [`seed_conformance`] with the OPTIONAL identity-host posture
...
/// id host (identity mode; RSS `docs/design/webid-outside-pod.md` →
/// `research/lws-design-records.md` §4). Subjects are on the IDENTITY origin (the served IRIs),
/// never the reserved store key. Carries exactly the provider-locked statements:
