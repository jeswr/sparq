...
//! path makes a single emit call after a successful mutation. A subscription is authorized per
//! resource: subscribe AND the receive upgrade both require `acl:Read` on the topic, through the same
//! `authz::WacAuthorizer` the LDP read path uses. Everything else network-facing (the live SPARQ HTTP
//! client, live JWKS) and the parts of the Solid surface that need designs not yet written (the
//! reconciler, multipart Range, `acl:agentGroup` resolution) are clearly marked `M2-next:` seams.
//! PATCH supports both the Solid N3 Patch and the `application/sparql-update` INSERT/DELETE-DATA
//! subset. The Solid conformance suite passes **41/41**
