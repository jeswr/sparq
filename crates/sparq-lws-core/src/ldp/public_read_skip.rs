...
//! Pre-crypto PUBLIC-READ skip (skip-crypto optimization, opt 3 of RSS `decisions/0002` →
//! `research/lws-design-records.md` §5).
//!
...
//! UNSAFE — and opt 1/2 — is recorded in full in RSS `decisions/0002` →
//! `research/lws-design-records.md` §5.) The `DPoP` header is ALSO a fall-through trigger so a
//! no-`Authorization` GET carrying a malformed `DPoP` header keeps the auth path's canonical `400`
//! (which it rejects even without `Authorization`) rather than being served as anonymous here.
//!
