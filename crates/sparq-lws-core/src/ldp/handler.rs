...
impl<S: Store> LdpState<S> {
    ...
    /// RSS `docs/design/backend-read-path.md` §3.1 → `research/lws-design-records.md` §7), still
    ...
    /// The shared write-path PLANNED authorization core (write-2, RSS
    /// `docs/design/backend-read-path.md` §3.1 → `research/lws-design-records.md` §7, applied to
    /// the write verbs): derive the ACL-candidate chain, fetch every candidate's presence/etag in
    /// ONE combined [`Store::read_plan`] round-trip (replacing the sequential k+1 per-candidate
    /// `meta` probes the [`WacAuthorizer::authorize`] walk pays), then decide via
    ...
    /// (`research/lws-design-records.md` §6, from RSS `decisions/0003`).
    ///
    ...
    /// `decisions/0003` (`research/lws-design-records.md` §6).
...
pub(crate) async fn serve_read<S: Store>(
    ...
    // V5 (RSS decisions/0003) — the membership-derived container `etag` computed above shifts on every
...
pub async fn put_handler<S: Store>(
    ...
    // WAC for PUT — EXISTENCE-NON-DISCLOSURE (RSS decisions/0003): a PUT requires `acl:Write` on the
    ...
    //    inheritable `acl:Write`; no row expects an Append-only PUT-create=201 — see RSS decisions/0003.)
    ...
    // V4 (RSS decisions/0003): a conditional precondition is a CONTENT/MEMBERSHIP-derived validator — a
...
pub async fn post_handler<S: Store>(
    ...
    if !container.is_container {
        // EXISTENCE-NON-DISCLOSURE (V6, RSS decisions/0003): the non-container existence branch (405 when a
...
pub async fn delete_handler<S: Store>(
    ...
    // V4 (RSS decisions/0003): a DELETE may carry `If-Match`/`If-None-Match`, whose 412-vs-2xx outcome
...
pub async fn patch_handler<S: Store>(
    ...
    // WAC for PATCH — EXISTENCE-NON-DISCLOSURE (RSS decisions/0003): the required mode is derived purely
    ...
    // delete-on-missing closure is now just the general rule. See RSS decisions/0003.)
    //
    ...
    // V4 (RSS decisions/0003) — the `solid:where` READ-gate. A patch carrying a `solid:where` clause READS
    ...
    // V4 (RSS decisions/0003): a conditional precondition is a CONTENT-derived validator — fold to the
...
/// from RSS `decisions/0003`). The prior mint returned
...
<#pwn> a acl:Authorization;
    ...
    async fn post_slug_dot_meta_is_allowed_meta_is_not_load_bearing() {
        ...
        assert_eq!(resp.status(), StatusCode::CREATED);
        // V2 (RSS decisions/0003): the minted `Location` is collision-INDEPENDENT — it CONTAINS the Slug
    ...
    async fn post_benign_slug_still_works_no_regression() {
        ...
        assert_eq!(resp.status(), StatusCode::CREATED);
        // V2 (RSS decisions/0003): the child's `Location` CONTAINS the Slug (`note`) as an opaque-suffixed
...
<#a> a acl:Authorization; acl:agent <{ALICE}>; acl:accessTo <https://pod.example/alice/c/secret>; acl:mode acl:Read, acl:Write, acl:Control."#
    ...
    // =====================================================================================
    // EXISTENCE-NON-DISCLOSURE (RSS decisions/0003) — the exhaustive byte-identical matrix.
    //
    ...
    // the true 404. See `guard_post_existence_requires_read` + RSS decisions/0003.
