...
//! (RSS `docs/design/backend-read-path.md` §3.4; RSS `docs/design/throughput-hard-cases.md` §5
//! `read-4-bodycache` — both → `research/lws-design-records.md` §7): a byte-budgeted LRU of
//! resource BODIES keyed by **`(blob_key, etag)`**, sat in front of
//! [`BlobStore::get`](super::blob::BlobStore::get) inside
//! [`CompositeStore`](super::CompositeStore) — so a hot, unchanged resource's repeat GET pays
//! **zero** blob-store round-trips (blob-gets/op 1 → 0 on a hit).
//!
