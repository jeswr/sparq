...
pub mod blob;
// The blob-body LRU cache (read-4 — RSS `docs/design/backend-read-path.md` §3.4 →
// `research/lws-design-records.md` §7): `(blob_key, etag)`-keyed, byte-budgeted, sat in front of
// BlobStore::get inside CompositeStore so a hot unchanged resource's repeat read pays zero blob
// round-trips. Immutable-by-construction (unique-per-write blob keys), so a hit can never be
// stale — see the module docs.
pub mod body_cache;
// Deterministic backend round-trip counters at the SparqClient/BlobStore seams (read-1 of the
// read-path perf plan — RSS `docs/design/backend-read-path.md` §7 →
// `research/lws-design-records.md` §7). Decorators used by the pinned counter tests + the bench
// harness; zero-cost when not wired in.
pub mod counting;
...
// [`embedded`] + RSS `decisions/0001-embed-sparq-in-process.md` →
...
pub trait Store: Send + Sync {
    ...
    /// ONE combined read-plan lookup for the read path (read-2 — RSS
    /// `docs/design/backend-read-path.md` §3.1 → `research/lws-design-records.md` §7): the
    /// target's authoritative metadata + the presence/etag of every ACL candidate, in a single
    /// index round-trip. See
...
/// [`BodyCache`] (read-4 — RSS `docs/design/backend-read-path.md` §3.4 →
/// `research/lws-design-records.md` §7) in front of the blob byte-fetch.
