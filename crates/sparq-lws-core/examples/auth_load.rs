...
struct Config {
    ...
    anon: bool,
    /// AUTHED-PUBLIC mode (EXPLORATORY only — RSS `decisions/0002` →
    /// `research/lws-design-records.md` §5): send a DPoP-bound token but target a PUBLIC resource.
    /// This INVERTS the privacy assertion — the pre-flight asserts the target IS public (anonymous
    /// GET 200) — so the sweep measures a PROOF-CARRYING GET of a public document.
    ///
    ...
    /// see RSS `decisions/0002` → `research/lws-design-records.md` §5). So this mode measures the
    /// UNCHANGED proof-carrying auth path; it is retained ONLY to demonstrate that the ES256 verify
    /// is the cost, NOT as a shippable result. The scenario is named `authed-public-doc`. Ignored
    /// in `anon` mode (the path opt-3 actually changes).
    expect_public: bool,
    duration: Duration,
    warmup: Duration,
...
async fn main() -> Result<(), String> {
    ...
    // a credentialed request — see RSS `decisions/0002`); `anon-public-doc` is the path opt-3
    // implements.
