...
const BATTERY: &[(&str, &str)] = &[
    (
        "select-all",
        "SELECT ?s ?p ?o WHERE { GRAPH ?g { ?s ?p ?o } } ORDER BY ?s ?p ?o",
    ),
    (
        "optional",
...
fn differential_battery_masked_equals_oracle_and_differs_from_unmasked() {
    ...
    for label in ["select-all", "count", "path"] {
        ...
        assert_ne!(
            ...
            "vacuous mask: {label} unchanged by masking"
        );
    }
...
fn fully_masked_graph_is_indistinguishable_from_absent() {
    ...
    assert_eq!(
        graphs.rows.len(),
        1,
        "only the notes graph remains enumerable"
    );
    assert!(!scoped
    ...
    assert!(scoped
        .ask("ASK { GRAPH <https://pod.ex/notes> { ?s ?p ?o } }")
        .unwrap());
}
...
fn scope_on_non_accessible_graph_does_not_widen() {
    ...
    assert_eq!(
        rows.rows.len(),
        0,
        "pattern scope widened a grant-less session"
    );
...
fn scoped_dataset_keeps_empty_default_graph_semantics() {
    ...
    assert_eq!(
        bare.rows.len(),
        0,
        "bare pattern must see the empty default graph"
    );
    let opt_in = scoped
        .query(
        ...
        .unwrap();
    assert!(
        !opt_in.rows.is_empty(),
        "union-default opt-in must range over the replica"
    );
}
