...
fn write_with_permission_succeeds_and_mutates() {
    ...
    s.update_as(&sess(Some(ALICE)), &insert_tag(doc, "alice"))
        .expect("alice may write priv0");
...
fn write_without_permission_is_denied_and_store_unchanged() {
    ...
    assert!(s
        .update_as(&Session::default(), &insert_tag(doc, "anon"))
        .is_err());
...
fn group_write_grant_allows_carol_and_bob_not_dave() {
    ...
    let before = graph_len(&s, doc);
    s.update_as(&sess(Some(CAROL)), &insert_tag(doc, "carol"))
        .expect("carol writes team2");
    assert_eq!(graph_len(&s, doc), before + 1);
    s.update_as(&sess(Some(BOB)), &insert_tag(doc, "bob"))
        .expect("bob writes team2");
    ...
    assert_eq!(
        graph_len(&s, doc),
        before + 2,
        "dave's denied write did nothing"
    );
}
...
fn delete_needs_write_not_just_append() {
    ...
    assert!(s
        .update_as(&sess(Some(ALICE)), &delete_title(doc, "doc 4-1-0-0"))
        .is_err());
    assert_eq!(graph_len(&s, doc), before);
}
...
fn alice_delete_with_write_succeeds() {
    ...
    s.update_as(&sess(Some(ALICE)), &delete_title(doc, "doc 0-4-0-0"))
        .expect("alice may delete priv0");
...
fn partial_graph_write_respects_per_graph_acls() {
    ...
    assert!(
        r.is_err(),
        "mixed-graph update denied (carol lacks priv0): {r:?}"
    );
    assert_eq!(
        graph_len(&s, ok_graph),
        before_ok,
        "writable graph untouched on deny"
    );
    assert_eq!(
        graph_len(&s, bad_graph),
        before_bad,
        "unwritable graph untouched"
    );

    // …and when carol writes ONLY the graph she may, it succeeds.
    s.update_as(&sess(Some(CAROL)), &insert_tag(ok_graph, "carol"))
        .expect("carol writes team2 alone");
...
fn default_graph_write_denied() {
    ...
    let r = s.update_as(
        &sess(Some(ALICE)),
        "INSERT DATA { <urn:x:s> <urn:x:p> <urn:x:o> }",
    );
    assert!(
        r.is_err(),
        "default-graph write denied even for owner: {r:?}"
    );
}
...
fn control_gates_acl_document_writes() {
    ...
    let doc = "https://pod.ex/priv0/c4/g0/d0.ttl";
    s.update_as(&sess(Some(ALICE)), &insert_tag(doc, "after-remat"))
        .expect("view survived remat");
}
...
fn variable_graph_target_fails_closed_for_non_owner() {
    ...
    assert!(
        r.is_err(),
        "variable-graph DELETE denied for non-omnipotent actor: {r:?}"
    );
}
...
fn clear_all_fails_closed() {
    ...
    assert!(
        r.is_err(),
        "CLEAR ALL denied (no actor can write every graph + every .acl): {r:?}"
    );
...
fn unparseable_update_is_an_error() {
    let mut s = wac_store();
    assert!(s
        .update_as(&sess(Some(ALICE)), "NONSENSE not sparql")
        .is_err());
}
...
fn var_graph_precise_allows_authorized_subset() {
    ...
    s.update_as(
        &sess(Some(CAROL)),
        &var_delete_title_under("https://pod.ex/team2/"),
    )
    ...
    assert_eq!(
        graph_len(&s, TEAM2_DOC),
        before - 1,
        "title deleted from a bound team2 graph"
    );
}
...
fn var_graph_precise_denies_readable_but_unwritable_binding() {
    ...
    let r = s.update_as(
        &sess(Some(CAROL)),
        &var_delete_title_under("https://pod.ex/mixed4/"),
    );
    assert!(
        r.is_err(),
        "carol may read but not write mixed4 -> denied: {r:?}"
    );
    assert_eq!(
        graph_len(&s, MIXED4_DOC),
        before,
        "denied variable-graph delete changed nothing"
    );
}
...
fn var_graph_precise_denies_when_a_bound_graph_is_unwritable_control_doc() {
    ...
    assert!(
        r.is_err(),
        "binding includes team2 .acl docs carol cannot control: {r:?}"
    );
    assert_eq!(graph_len(&s, acl), before_acl, ".acl untouched on deny");
    assert_eq!(
        graph_len(&s, TEAM2_DOC),
        before_doc,
        "content untouched on deny (check is pre-apply)"
    );
}
...
fn var_graph_precise_insert_authorized_subset_succeeds() {
    ...
    s.update_as(&sess(Some(CAROL)), &upd)
        .expect("precise insert into team2 graphs carol owns");
    assert_eq!(
        graph_len(&s, TEAM2_DOC),
        before + 1,
        "one tag inserted into a bound team2 graph"
    );
}
...
fn var_graph_precise_insert_into_unwritable_binding_denied() {
    ...
    assert!(
        r.is_err(),
        "insert into unwritable mixed4 graphs denied: {r:?}"
    );
...
fn var_graph_empty_binding_is_a_permitted_noop() {
    ...
    s.update_as(&Session::default(), &upd)
        .expect("empty-binding variable-graph update is a no-op");
}
...
fn var_graph_with_clause_precise_still_denies_unwritable_binding() {
    ...
    assert!(
        r.is_err(),
        "WITH does not waive write-auth: carol may read but not write mixed4 -> denied: {r:?}"
    );
    assert_eq!(
        graph_len(&s, MIXED4_DOC),
        before,
        "denied WITH variable-graph delete changed nothing"
    );
}
...
fn var_graph_with_clause_denies_binding_to_auth_view() {
    ...
    assert!(
        before > 0,
        "materialized auth view holds the WAC grant triples"
    );
    ...
    assert_eq!(
        graph_len(&s, auth),
        before,
        "denied op left the auth view untouched"
    );
}
