...
- **Triples-native + zero-copy enforcement** — pods, ACL/ACR docs, and the auth view are all ordinary
  ...
  zero-copy dataset view, with a v1 `FROM NAMED` rewrite as a standard-SPARQL portability path. Opt-in `solid-sparql-query` (OFF by default, byte-identical when off) adds the [solid-sparql-query](https://github.com/jeswr/solid-sparql-query) Editor's Draft §4 union-default-graph opt-in (`FROM <…#union-default-graph>`) — see the SKILL (#992 item 4).
- **Write-path gating, WAC-Allow, per-resource decision + ACL write-through (#992 FR-1/3/5/6/7)** — `update_as` gates writes; `put_acl`/`delete_acl` are the authoritative LDP-`PUT`/`DELETE`-on-`.acl` write-through (atomic graph replace + re-materialize, fail-closed rollback; a single-`.acl` write **scopes** session-cache invalidation to that ACL's origin so other pods stay warm, and per-request `decide` costs one pod's grants, not the whole store — provably equal to a full rebuild, `sq-b7k7u`); `wac_allow` builds the header; `decide`/`decide_batch`/`resolve_acl` answer the per-request *"may X do M on R?"* with the
