...
- **Pattern-scoped masking (opt-in `pattern-scope`, spike `sq-lrtc3.3`)** — an ODRL target as allow/deny **triple patterns** over a
  graph, enforced by **materializing the masked sub-graph replica** (masked triples physically absent ⇒ oracle-equivalent under `OPTIONAL`/`EXISTS`/`MINUS`/aggregates/`COUNT` by construction). Replicas are cached per **(graph name, normalized scope)** in a bounded sharded cache and dropped by every write path, so sessions sharing a scope class share one build; unmasked graphs are forked, not rebuilt. Design: `research/odrl-pattern-scoped-targets-2026-07.md`; envelope: `bench/pattern-scope/`.
