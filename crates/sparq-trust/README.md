...
- **Cost / decidability bound** (`cost`, **opt-in `cost-bound`**, `sq-pfae.9` / [#3281](https://github.com/jeswr/sparq/issues/3281)) — the P8 spike the design deferred. `admission_cost_bound` states the closed-form worst case for `admit` — a **single pass**, never a fixpoint: `2 + 4R + 3R·T` gate operations for `R` rules over a `T`-triple graph — and `admit_measured` checks it against the REAL gate through the operation meter `admit` already threads (**no `cfg` fork**), pinned **tight** from both sides so it can be neither inflated nor deflated unnoticed. `analyse_seeding` / `require_one_side_bound` decide **one-side-bound seeding** + head range-restriction over `.acr` N3 rule text, fail-closed on anything they cannot parse. Honest finding: design §3.1's ABAC rule and the `admissibility` discharge rule ARE one-side-bound; the transitive-closure rule is a **two-unbound-atom seed**, safe only because its extent is the bundled constant fact base. **Deterministic counts + static analysis only — no clock sampled** (work-box / EC2 wall-clock is non-canonical and is never gated).
