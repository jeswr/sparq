...
- **Holder-side trust-expression evaluation** (`expression`, **opt-in, part of `sq-6syab.4`**) —
  the clear-path contract evaluation: trust-requirements parsing, reference rewrite (Q→Q'), scoped query evaluation, provenance-encoded response assembly with verification re-check and status-attestation bridging.
