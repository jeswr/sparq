...
use sparq_engine::{
    ask, ask_prepared, construct_ntriples, construct_prepared, count, count_prepared, describe_prepared,
    query, query_json, query_json_prepared, query_prepared, query_prepared_with_budget, PreparedQuery,
    QueryBudget,
};
...
fn prepared_select_matches_string_path_across_graphs_and_runs() {
    ...
    for graph in [g(5), g(9)] {
        ...
        assert_eq!(query_json_prepared(&graph, &prepared).unwrap(), query_json(&graph, q).unwrap());
        assert_eq!(count_prepared(&graph, &prepared).unwrap(), count(&graph, q).unwrap());
    }
}
...
fn prepared_ask_construct_describe() {
    ...
    assert_eq!(ask_prepared(&graph, &ask_q).unwrap(), ask(&graph, "PREFIX : <http://ex/> ASK { :s1 :p ?o }").unwrap());
    ...
    assert!(query_json_prepared(&graph, &ask_q).unwrap().contains("\"boolean\":true"));
    ...
    assert_eq!(sparq_engine::triples_to_ntriples(&triples), construct_ntriples(&graph, con).unwrap());
...
fn prepared_respects_dataset_clause_and_budget() {
    ...
    let tight = QueryBudget { max_rows: Some(3), ..QueryBudget::unlimited() };
