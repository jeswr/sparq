...
fn arb_literal(mask: DataMask) -> BoxedStrategy<T> {
    let mut opts: Vec<BoxedStrategy<T>> = vec![
        ...
        prop_oneof![Just(0i64), Just(1), Just(-1), Just(1000)]
            .prop_map(T::Int)
            .boxed(),
        ((-3000i128..=3000), (1u32..=2))
            .prop_map(|(m, s)| T::Dec(m, s))
            .boxed(),
        proptest::sample::select(&STR_POOL[..])
            .prop_map(|s| T::Str(s.to_string()))
            .boxed(),
        (
            proptest::sample::select(&STR_POOL[..]),
...
fn arb_qseed() -> impl Strategy<Value = QSeed> {
    (
        ...
        .prop_map(
            |(body, bind, filter, distinct, proj_mask, order, limit)| QSeed {
                body,
                bind,
                filter,
                distinct,
