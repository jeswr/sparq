...
**leakage-envelope assembly + dual ratification**; Phase 6 the **result-aware
source-combination prune** — the unsatisfiable-conjunct collapse (sq-pwr.3) and the FedUP-style
**quotient-summary provenance** prune (sq-xkrt). The remaining privacy-bearing phases
...
* [`assemble_leakage_envelope`] + [`ratify_envelope`] (**Phase 4, implemented**) — the
  ...
  [`envelope`].
* [`prune_source_combinations`] (**Phase 6 Rule C1, implemented**) — the **result-aware
  ...
  **provably contribute no answer** so the seam routes fewer toward MPC. The rule expressible
  from the `SourceDescriptor` summary alone is **unsatisfiable-conjunct collapse**: if any
  pattern's Phase-2 candidate list is empty, that conjunct is a proved-empty relation, so the
  whole conjunction is unsatisfiable and **every** source-combination is dead (`∅ ⋈ R = ∅`). It
  carries the Phase-2 selection through unchanged (advisory + auditable, never silently
  dropping), reporting `bgp_satisfiable`, the empty-pattern witnesses, the BGP join components,
  and a combination-dead audit trail. A value-overlap prune *from the `SourceDescriptor`* is
  **declined** as not recall-safely expressible (documented in [`combination`]). It is selection
  plumbing — **not** a cryptographic guarantee; runs NO MPC, makes NO privacy/soundness claim.
* [`prune_source_combinations_with_summaries`] + [`SourceQuotientSummary`] (**Phase 6 Rule C2,
  implemented**) — the FedUP-style (WWW'24) **provenance over quotient summaries** prune, the
  actual lever the design record's §3 names as the highest-leverage pre-MPC cost win. A source
  may publish a [`SourceQuotientSummary`] — its graph under the **authority quotient**, with
  literal values never recorded — and declare it a *complete over-approximation*. The pass then
  evaluates the BGP at the quotient level once per source-combination and drops the combinations
  whose evaluation is empty, which **provably** produce no concrete answer; that kills
  combinations whose patterns are individually non-empty but *jointly* unsatisfiable (disjoint
  authorities across a join), which Rule C1 cannot see. Recall-safe in three layers: a source
  with no complete summary constrains nothing, the over-approximation makes an empty evaluation
  a proof, and every indecisive case (a predicate/term variable-domain clash, an over-budget
  combination space via [`CombinationBudget`], an over-wide join) **declines** and prunes
  nothing — all reported in [`SummaryPruneOutcome`]. It reads only public, source-published
  metadata; runs NO MPC and makes NO privacy/soundness claim. See [`combination`] and
  [`quotient`].
...
MPC-routing seam (beads sq-2q1x + sq-fix4 + sq-i1wh2 + sq-pwr.2 + sq-pwr.3 + sq-xkrt). See `README.md`
...
`ratify_envelope` gate, and the Phase-6 `prune_source_combinations` /
`prune_source_combinations_with_summaries` result-aware combination prune over
`SourceQuotientSummary`) is gated behind the
...
pub mod combination;
// [OPUS-4.8] sq-xkrt: Phase 6's quotient-summary INPUT (the FedUP provenance lever) lives in its
// own module — it is a source-declared record, orthogonal to the prune that consumes it.
#[cfg(feature = "fedplan-mpc")]
pub mod quotient;
...
// [OPUS-4.8] sq-pwr.3 / sq-xkrt: Phase 6 — the result-aware source-combination prune (Rule C1 +
// the quotient-summary provenance Rule C2) and its result types.
#[cfg(feature = "fedplan-mpc")]
pub use combination::{
    prune_source_combinations, prune_source_combinations_with_summaries, BgpComponent,
    CombinationBudget, CombinationPruneReason, PrunedCombination, PrunedCombinations,
    SourceCombination, SummaryDeclineReason, SummaryPruneOutcome,
};
// [OPUS-4.8] sq-xkrt: Phase 6's quotient-summary input (the FedUP provenance lever).
#[cfg(feature = "fedplan-mpc")]
pub use quotient::{
    quotient_iri, QuotientTerm, QuotientTriple, SourceQuotientSummary,
    SourceQuotientSummaryBuilder, SummaryTerm,
};
