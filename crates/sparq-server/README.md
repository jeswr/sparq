...
`503`** (never acked, writer stays alive — fail-closed), a contract `tests/persist.rs` exercises over
HTTP for a transient burst, a permanent jam, and an interleaved recover→fail→recover sequence (the
final durable set is exactly the successful writes; no refused write resurrects). The persisted delta
is the resolved acked value, so non-deterministic updates (`NOW()`/`RAND()`/`LOAD`) persist exactly
what the client saw.
