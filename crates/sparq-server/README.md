...
- **Opt-in features** (a build without a feature carries zero code for it) — `time-travel`
  (EXTENDS `?generation=N` retention past the default concurrency window), `point-in-time` ([OPUS-5] sq-lsp7k.7 — `?at=<xsd:dateTime | unix-nanos>` serves the store as of an ARBITRARY past instant, rebuilt by reverse-replaying the durable change stream, so it reaches past the in-memory ring; implies `time-travel`+`change-stream` and needs `--change-stream DIR`; fail-closed `410` past the log's retention horizon), `geo` (`geof:` functions), `service` (SERVICE federation,
