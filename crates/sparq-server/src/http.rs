...
impl ChangeStreamState {
    ...
    /// [OPUS-5] (sq-iz7ag, sq-lsp7k.7) The read handle's trim horizon — the oldest seq the durable
    /// log still retains (`0` until retention drops a segment, then the first seq of the oldest
    /// survivor). `ChangeLog::poll` fails CLOSED below this offset, so a whole-retained-history
    /// read — `GET /streams` at `TRIM_HORIZON`, and the point-in-time path — polls from exactly
    /// here rather than guessing an offset that a trim may already have invalidated.
...
impl AppState {
    ...
    /// anchor below it. The point-in-time path (`sq-lsp7k.7`) polls the whole retained history
    /// from it for the same reason. `None` when the change-stream is not configured.
    ///
...
async fn resolve_point_in_time(state: &AppState, raw: &str) -> Result<RequestPin, Response> {
    ...
    let current = state.current();
    // Poll from the trim horizon, not from 0: `ChangeLog::poll` fails CLOSED below the oldest
    // retained seq, so a trimmed log must be read from exactly where its history still starts.
    let first_seq = state.change_stream_trim_horizon().unwrap_or(0);
