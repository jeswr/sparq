...
impl Metrics {
    ...
    pub(crate) fn record_request(&self, endpoint: &'static str, status: u16, elapsed: Duration) {
        *self.requests.lock().expect("metrics lock poisoned").entry((endpoint, status)).or_insert(0) += 1;
        if endpoint == "/sparql" {
            ...
            self.query_sum_nanos.fetch_add(elapsed.as_nanos() as u64, Ordering::Relaxed);
        }
    }
    ...
    pub(crate) fn render(&self, active_subscriptions: usize, graph_triples: usize) -> String {
        ...
        for ((endpoint, status), n) in self.requests.lock().expect("metrics lock poisoned").iter() {
            let _ = writeln!(out, "sparq_http_requests_total{{endpoint=\"{endpoint}\",status=\"{status}\"}} {n}");
        }
        ...
        let _ = writeln!(out, "sparq_query_duration_seconds_bucket{{le=\"+Inf\"}} {count}");
        let _ = writeln!(
            out,
        ...
        out.push_str("# HELP sparq_active_subscriptions Currently active WebSocket subscriptions.\n");
        ...
        let _ = writeln!(out, "sparq_updates_total {}", self.updates_total.load(Ordering::Relaxed));
        out
    }
}
...
pub(crate) async fn track(State(state): State<AppState>, req: Request, next: Next) -> Response {
    ...
    state.metrics().record_request(endpoint, resp.status().as_u16(), start.elapsed());
    resp
}
...
mod tests {
    ...
    fn exposition_shape_and_histogram_cumulativity() {
        ...
        assert!(text.contains("sparq_http_requests_total{endpoint=\"/sparql\",status=\"200\"} 2"), "{text}");
        assert!(text.contains("sparq_http_requests_total{endpoint=\"/sparql\",status=\"400\"} 1"), "{text}");
        assert!(text.contains("sparq_http_requests_total{endpoint=\"/health\",status=\"200\"} 1"), "{text}");
        // Buckets are cumulative: 0.001 holds the 100µs request; 0.005 adds 2ms;
        // 0.1 and up hold all three; +Inf == _count.
        assert!(text.contains("sparq_query_duration_seconds_bucket{le=\"0.001\"} 1"), "{text}");
        assert!(text.contains("sparq_query_duration_seconds_bucket{le=\"0.005\"} 2"), "{text}");
        assert!(text.contains("sparq_query_duration_seconds_bucket{le=\"0.1\"} 3"), "{text}");
        assert!(text.contains("sparq_query_duration_seconds_bucket{le=\"+Inf\"} 3"), "{text}");
        assert!(text.contains("sparq_query_duration_seconds_count 3"), "{text}");
