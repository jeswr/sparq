//! **Vector quantization** for large collections: shrink the per-vector memory footprint so
//! 100M-scale sources fit a candidate cache in RAM, at a measured cost in recall. Two encoders,
//! both round-trippable and both able to *estimate* a distance from the compressed codes alone:
//!
...
//!
//! # Cosine convention (matches [`crate::graph`])
//!
...
//! [`DistanceTable::new`] normalizes the query — so a caller can hand raw source vectors straight
//! in and the resulting ranking is cosine, identical to the graph searcher. The `cosine_*` helpers
//! convert an estimated `d²` back to a cosine score for callers that want one.
//!
//! [`fit`]: ProductQuantizer::fit
//! [`encode`]: ProductQuantizer::encode
//!
//! # Intended use with [`VamanaIndex`](crate::graph::VamanaIndex)
//!
//! This module is the **PQ-compressed in-RAM candidate cache** that closes the plain Vamana
//! graph's honest gap to *full* DiskANN (see [`crate::graph`]'s "Honest scope" note). The wiring
//! [`VamanaIndex::build_with_pq`](crate::graph::VamanaIndex::build_with_pq) drives:
//!
//! 1. After building the `.spqg` graph, [`fit`](ProductQuantizer::fit) a [`ProductQuantizer`] on
//!    the source's vectors and [`encode_store`](ProductQuantizer::encode_store) every vector into
//!    a flat `count × M`-byte code array — small enough to stay resident (e.g. 100M × 16 B =
//!    1.6 GB where the f32 vectors would be 100M × dim × 4 B).
...
//! 3. The codebook + PQ params persist alongside the graph in a trailing `.spqg` section; the
//!    header's 4 reserved bytes carry an encoding tag, so it is a backwards-compatible addition.
...
/// direction). Source vectors are never zero (see [`VectorSource`]'s contract); a degenerate
...
/// auto-vectorizes — the same shape as [`crate::graph`]'s `sq_dist`.
...
/// (`cos = 1 − d²/2`). The inverse of the mapping the graph searcher uses, so a quantized estimate
/// is directly comparable to its scores.
...
impl ScalarQuantizer {
    ...
    /// [`VectorSource::iter`]), paired with the slot→id mapping so a caller can map a ranked slot
    /// back to its id. The 4×-smaller candidate cache. Errors on a source/quantizer dim mismatch.
    pub fn encode_store<S: VectorSource + ?Sized>(
        &self,
        store: &S,
    ) -> Result<EncodedStore, String> {
...
impl ProductQuantizer {
    ...
    /// Serializes the codebook to a self-describing little-endian byte block
    ...
    /// matching [`VectorSource::iter`]) plus the slot→id mapping. This is the in-RAM candidate
    /// cache for [`VamanaIndex`](crate::graph::VamanaIndex). Errors on a dim mismatch.
    pub fn encode_store<S: VectorSource + ?Sized>(
        &self,
        store: &S,
    ) -> Result<EncodedStore, String> {
...
/// A source's vectors encoded to fixed-length codes (SQ or PQ) plus the slot→id mapping — the
/// **in-RAM candidate cache**. `codes` is `len × stride` bytes in slot order (matching
/// [`VectorSource::iter`]); `stride` is `dim` for [`ScalarQuantizer`] or `M` for
/// [`ProductQuantizer`]. Ranking over this cache then re-ranking the top candidates against the
/// full-precision vectors (or [`VamanaIndex`](crate::graph::VamanaIndex)'s mmap) is the DiskANN
/// search loop.
#[derive(Clone, Debug)]
...
impl EncodedStore {
    /// Builds an [`EncodedStore`] from a slot→id mapping and the flat `len × stride` code array
    /// (slot order) — the inverse of the `ids`/`codes` an encoder produces, used to reload a
    /// persisted candidate cache. Errors if `codes.len()` is not `ids.len() × stride` (or
    /// `stride == 0` with codes present), so a malformed block can't produce out-of-bounds
    /// [`code`](Self::code) reads.
    pub fn from_parts(
        ids: Vec<VectorId>,
        codes: Vec<u8>,
        stride: usize,
    ...
    /// The caller's id at `slot`.
    pub fn id(&self, slot: usize) -> VectorId {
        self.ids[slot]
    }
    ...
    /// The flat `len × stride` code array (slot order) — for persisting the candidate cache;
    /// pair with [`from_parts`](Self::from_parts) to reload it.
    pub fn codes(&self) -> &[u8] {
        &self.codes
    }
    /// Iterates `(slot, id, code)` in slot order.
    pub fn iter(&self) -> impl Iterator<Item = (usize, VectorId, &[u8])> + '_ {
    ...
    /// [`VamanaIndex`](crate::graph::VamanaIndex) would re-rank on disk. Ties break on ascending
    /// id. The caller must pass a table built from the same [`ProductQuantizer`] that produced
    /// these codes (the `stride` must equal the table's `M`); a mismatch panics in
    /// [`DistanceTable::distance`].
    pub fn rank_pq(&self, table: &DistanceTable, k: usize) -> Vec<(VectorId, f32)> {
