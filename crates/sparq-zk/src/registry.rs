...
/// `ecdsa-rdfc-2019`, `bbs-2023`, `ecdsa-sd-2023`, …) the credential carried
/// before it was off-circuit verified (or, for the selective-disclosure seam,
/// ingested WITHOUT a soundness-bearing check) and re-committed. It is **NOT** a
/// re-verifiable in-proof property (§5.3): the query proof does not re-check the
/// VC's Data-Integrity signature. Recorded as a plain literal (the verbatim
/// `proof.cryptosuite` value, which is a bare suite token in a W3C VC, not an
/// absolute IRI).
// [OPUS-4.8] sq-9c5e.
...
pub struct RegistryEntry {
    ...
    /// or the selective-disclosure `bbs-2023`), set when the entry was produced by
    /// the off-circuit VC ingest bridge (`crate::vc_bridge`). Back-compat / perf-
    /// comparison provenance only — NOT a re-verifiable in-proof property; distinct
    /// from [`Self::cryptosuite`] (the sparq scheme the query proof binds to).
    /// `None` for non-VC-bridge entries.
    // [OPUS-4.8] sq-9c5e.
...
impl RegistryEntry {
    ...
    /// source VC (e.g. `eddsa-rdfc-2022`, or the selective-disclosure `bbs-2023`).
    /// This is back-compat / perf-comparison provenance ONLY; it does not change
    /// [`Self::cryptosuite`] (the sparq scheme the query proof binds to) and is not
    /// a re-verifiable in-proof property.
    // [OPUS-4.8] sq-9c5e.
    #[must_use]
...
mod tests {
    ...
    fn source_cryptosuite_round_trips_through_store() {
        ...
        // A selective-disclosure provenance token (sq-u5y1f) round-trips too.
        let sd = RegistryEntry::new(
            NamedNode::new("https://dmv.example/vc/lic-sd").unwrap(),
            Fr::from(11u64),
            salt_from_bytes(&[3u8; 32]),
        )
        .with_source_cryptosuite("bbs-2023");
        let mut gsd = Graph::load_str("", "turtle").unwrap();
        install_registry(&mut gsd, std::slice::from_ref(&sd)).unwrap();
        assert_eq!(read_registry(&gsd)[0].source_cryptosuite.as_deref(), Some("bbs-2023"));
