...
//!   the `zk:sourceCryptosuite` provenance (sq-9c5e, design §5), plus the
//!   selective-disclosure ingest SEAM (`bbs-2023` / `ecdsa-sd-2023`, sq-u5y1f)
//!   which re-commits + records provenance but asserts NO SD soundness (design §5.3).
//!
...
// the query proof does NOT re-verify the VC proof in-circuit (design §5.3). The
// selective-disclosure INGEST SEAM (`bbs-2023` / `ecdsa-sd-2023`, sq-u5y1f) lives
// in the same module and asserts NO SD soundness (no real BBS verifier in-repo,
// design §5.3). NOT externally audited (sq-qhy4).
