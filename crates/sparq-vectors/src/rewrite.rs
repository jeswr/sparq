...
fn derive_mask(
    ...
    // [OPUS-4.8] (sq-36ol) The mask the constraining sub-BGP derives is a pure function of
    // (the sub-BGP, the graph) — re-deriving it for the SAME sub-BGP against the SAME graph
    // always yields the same ids. So cache it keyed by (sub-BGP terms, graph fingerprint).
    // The fingerprint folds dict_len + triple_count + an id-ordered content hash (see
    // `crate::fingerprint`), so ANY mutation that could change which ids the sub-BGP binds —
    // an added/removed triple, a shifted dict id, a changed term — changes the fingerprint
    // and therefore MISSES the cache (recompute). The query variable `?node` is normalised to
    // a fixed name so two queries that differ only in the neighbour variable's name share the
    // entry (the derived id-set is independent of that name). A cache HIT returns an `IdMask`
    // byte-identical to a fresh derivation; a MISS recomputes through the engine as before.
