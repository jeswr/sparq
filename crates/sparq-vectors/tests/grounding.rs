...
use sparq_vectors::{
    fuse_rrf_weighted, ground, sketch_predicate, Grounding, GroundingConfig, Modality, OutputType,
    VectorStore,
};
...
ex:trusted pkg:assurance secx:Proven      ; pkg:confidence "1.0" .
ex:dubious pkg:assurance secx:Conjectured ; pkg:confidence "0.2" .
"#;
    let g = Graph::load_str(PROV, "turtle").unwrap();
    ...
    // ON arm: the trusted neighbour's contribution dominates.
    ...
    assert!(
        pooled[0] > pooled[1],
        "the higher-provenance neighbour must dominate the sketch ({:?})",
        pooled
    );
    let wt = pw.weight_for_subject(trusted);
    let wd = pw.weight_for_subject(dubious);
