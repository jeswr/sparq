...
mod tests {
    ...
    #[test]
    fn dt_compare_symmetry_property() {
        // INVARIANT: flip(dt_compare(a, b)) == dt_compare(b, a)
        // This structural symmetry holds because the implementation is explicitly
        // built around the flip() helper across three exhaustive cases.

        // (1) Both timezoned distinct instants: order reverses.
        let tz1 = dt("2020-01-01T10:00:00Z");
        let tz2 = dt("2020-01-01T15:00:00Z");
        assert_eq!(dt_compare(&tz1, &tz2), TemporalOrder::Less);
        assert_eq!(flip(dt_compare(&tz1, &tz2)), dt_compare(&tz2, &tz1));
        assert_eq!(dt_compare(&tz2, &tz1), TemporalOrder::Greater);

        // (2) Both timezone-less distinct: order reverses.
        let local1 = parse_datetime("2020-01-01T08:00:00", DT).unwrap();
        let local2 = parse_datetime("2020-01-01T12:00:00", DT).unwrap();
        assert_eq!(dt_compare(&local1, &local2), TemporalOrder::Less);
        assert_eq!(
            flip(dt_compare(&local1, &local2)),
            dt_compare(&local2, &local1)
        );
        assert_eq!(dt_compare(&local2, &local1), TemporalOrder::Greater);

        // (3) Mixed tz-vs-local OUTSIDE ±14h window: definite order reverses.
        // Timezone-less "2020-01-05T00:00:00" vs timezoned "2020-01-01T00:00:00Z"
        // (4 days = 345,600 seconds >> 14*3600 = 50,400 seconds)
        let local_far = parse_datetime("2020-01-05T00:00:00", DT).unwrap();
        let tz_near = dt("2020-01-01T00:00:00Z");
        assert_eq!(dt_compare(&local_far, &tz_near), TemporalOrder::Greater);
        assert_eq!(
            flip(dt_compare(&local_far, &tz_near)),
            dt_compare(&tz_near, &local_far)
        );
        assert_eq!(dt_compare(&tz_near, &local_far), TemporalOrder::Less);

        // (4) Mixed tz-vs-local WITHIN ±14h window: Indeterminate is self-symmetric.
        // Timezone-less "2020-01-01T06:00:00" vs timezoned "2020-01-01T00:00:00Z"
        // (6 hours = 21,600 seconds < 14 hours = 50,400 seconds -> Indeterminate)
        let local_close = parse_datetime("2020-01-01T06:00:00", DT).unwrap();
        let tz_ref = dt("2020-01-01T00:00:00Z");
        assert_eq!(
            dt_compare(&local_close, &tz_ref),
            TemporalOrder::Indeterminate
        );
        assert_eq!(
            flip(dt_compare(&local_close, &tz_ref)),
            dt_compare(&tz_ref, &local_close)
        );
        assert_eq!(
            dt_compare(&tz_ref, &local_close),
            TemporalOrder::Indeterminate
        ...
        // (5) Reflexivity: any value equals itself (flip(Equal) = Equal).
        let a = dt("2020-06-15T12:30:45.123Z");
        assert_eq!(dt_compare(&a, &a), TemporalOrder::Equal);
        assert_eq!(flip(dt_compare(&a, &a)), dt_compare(&a, &a));

        let b = parse_datetime("2020-06-15T12:30:45.123", DT).unwrap();
        assert_eq!(dt_compare(&b, &b), TemporalOrder::Equal);
        assert_eq!(flip(dt_compare(&b, &b)), dt_compare(&b, &b));
    }
}
