...
| #5367 | 2026-08-02 | the durable CodeQL posture is RECORDED, not flipped: `codeql.yml` stays on `main` byte-identical and `disabled_manually`, the outage becomes tracked gap **GX-OSSF-4** with its compensating controls and residual risk named, and the OpenSSF self-cert answers that cited a live CodeQL are corrected (`static_analysis_common_vulnerabilities` → honest **Unmet**) | of the issue's two options only this one is deliverable by a commit — a `disabled_manually` workflow stays disabled until a maintainer runs `gh workflow enable`, so PR #3427's "re-enable as advisory" can ship only its advisory-registry half; leaving the outage implicit while waiting for a PR that structurally cannot land it was the actual defect | `docs/branch-protection.md` §CodeQL SAST — the durable posture · `compliance/openssf/gap-register.md` GX-OSSF-4 | adopted |
