...
| #2933 | 2026-09-02 | resolve the #1206 exec-model fork: adopt NEITHER a Volcano pull model (a) nor per-page `ORDER BY`/`LIMIT`/`OFFSET` re-evaluation (b) from the GUI; instead bound the EXISTING cursor pull by the display row cap (`streamQueryRows`'s `maxRows`), and keep the evaluation half gated on (a) as engine work | (a) is an engine-core rewrite, not a GUI change; (b) is page-correct only under a TOTAL solution order, which SPARQL's partial `ORDER BY` does not give and the GUI cannot certify (no wasm-exposed parser or algebra to rewrite against), so it can silently skip or duplicate rows across pages; but the wasm cursor already IS a pull iterator over the materialised result, so the SERIALISATION half of the per-page cost can be made demand-driven soundly today | #2933 · `research/gui-design.md` §A.5.1 · `packages/sparq-client/test/stream-query-rows.test.mjs` | adopted |
| #1206 | 2026-06-23 | ship GUI result pagination (the rendering half) now; gate the demand-driven page-wise EVALUATION half behind a pull-iterator exec-model decision | the wasm cursor slices already-materialised rows, so real lazy eval needs a Volcano iterator or an order-dependent rewrite | #1205 · `research/gui-design.md` §A.5.1 | adopted (the gate it set is resolved by #2933) |
