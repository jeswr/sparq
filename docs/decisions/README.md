...
| #5692 | 2026-08-03 | CI runner-fold candidate (sq-6vshe.7) DECLINED: `site-e2e-foundation.yml` · `determinism-gate` stays its own job rather than folding into `a11y-ratchet` | the gate is hermetic (checkout + grep) and the fold target is a browser lane (`npm ci` + `playwright install`), so a fold makes a hard gate's verdict conditional on the npm registry/CDN; it re-nests, days later, the gate #3773 deliberately un-nested; and, unlike the #5221 fold, both check-runs gate here, so merging them does coarsen a gating verdict. Saving was one runner claim on a `site/**`-only trigger. The "a demotion would silently un-gate it" argument was checked and NOT relied on — C3 of `check-advisory-registry.py` catches it | `research/ci-runner-consolidation-2026-07.md` §8.1 · the in-situ comment on the job | adopted |
