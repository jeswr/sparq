...
> (e.g. the lint gate is named `fmt + clippy (gate)`, *not*
...
| `fmt + clippy (gate)` | `cargo fmt --all --check` + `cargo clippy --workspace --all-targets -- -D warnings`; both checks are hard gates. [SONNET-4.6] |
