...
- **Confirm the isolated-builder provenance verified** (issue #4571 / GX-11 / SL-B3-b). Publishing
  the Release fires `.github/workflows/release-verify.yml`, which re-downloads every published
  ...
  run). Its uploaded `provenance-verification.log` is the evidence record. To run the identical
  checks by hand — e.g. to re-verify an older tag, or from a consumer's machine:
