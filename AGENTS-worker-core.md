...
5. **Model provenance (model-aware).** Stamp the model that ACTUALLY authored the change: an inline `[MODEL]` marker on new code/notes **+** a `Co-Authored-By: Claude MODEL <noreply@anthropic.com>` commit trailer, using whichever model you are running as — `[OPUS-4.8]` + `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>` while Opus is the fallback, and `[FABLE]` / `[SONNET-4.6]` / `[HAIKU-4.5]` + the matching trailer when a tier runs as that model.
