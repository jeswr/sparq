...
- **Periodic sweep:** `git worktree prune` + remove stale worktrees; if disk is tight this is the first lever (before launching EC2 — see *Maximise parallelism* — or asking the user). `target/` dirs and `bench/*` data are git-ignored and regenerable, so they are safe to delete; never delete tracked scripts or `.gitignore`.
