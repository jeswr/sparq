...
**Perf gate — deterministic strict, timing noise-robust (`scripts/perf-gate.py`, sq-dzfu).** The perf ratchet hard-gates the **DETERMINISTIC** metrics (integer byte counts — `store_bytes_per_triple{,_small}`, `dict_bytes_per_term`, `wasm_bundle_bytes`) strictly against the committed best-ever floor in `bench/perf-baseline.json`: any value past its band fails. The **TIMING** metric (`parse_ns_per_byte`, wall-clock-derived) is made robust to GitHub-runner noise via **best-of-N re-measure** — on a band trip the gate re-runs the cheap `ci-bench.sh --parse-only` path up to K times and keeps the best (min) reading, failing only if the best still regresses (runner noise only inflates a wall-clock reading, so a one-off spike is squeezed out while a real parse regression — slow on every reading — still fails). The deterministic-vs-timing split is data-driven from each metric's `mode` (`noise`=timing, else deterministic), not a hard-coded name list. Result: CI-runner timing noise never blocks a merge, real regressions are still caught (mirrors PR #62's coverage re-measure fix).
...
**Read the verdict ASYNC — never block on `--wait`.** <!-- [OPUS-4.8] charter cross-poll #76/#122 --> Because the post-commit hook already auto-enqueues the review, read the verdict asynchronously with `roborev show <sha>` (poll briefly — `roborev list` / `roborev status` — if the daemon hasn't finished) rather than sitting on a blocking `roborev review <sha> --local --wait`, which stalls the orchestrator loop. The verdict must still PASS before a branch merges (see below); you just don't hold a foreground call to get it. (Consistent with *Orchestration cadence — background by default*.)
...
**Deploy-image hygiene (data-exfil guardrail — not overridable).** <!-- [OPUS-4.8] charter cross-poll #121 --> sparq publishes the `sparq-server` container to `ghcr.io` (the Dockerfile base is SHA-pinned — see *Scorecard supply-chain* above). A harness-level safety classifier hard-blocks `docker build . --push <external-registry>` when the build **context is the repo root** — repo contents and any local secrets could ride into image layers, a data-exfiltration pattern — and the block stands even with explicit user authorization. So **never `docker build --push` with the repo root as context.** Two portable workarounds: (a) build from a **minimal artifact-only context** — just the compiled server binary + its runtime config, never the source tree — then push; or (b) **build on the target host** so the image never leaves the box (push the source branch to the host's remote, `docker build` + repin the digest there). This is a harness guardrail (not a per-repo setting), so it applies to any image push regardless of authorization — know the workaround before you hit the block.
