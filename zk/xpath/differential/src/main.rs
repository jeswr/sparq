...
//!
//! Where a cross-check fires, the case is NOT silently dropped: it is recorded in
//! [`DIVERGENCES`], emitted into the generated file as a commented-out assertion carrying
//! the spec-correct value, and pinned by a unit test here so the workaround expires the
//! day sparq-engine is fixed.
//!
...
/// engine bug and paint the (correct) circuit red, so these cases are emitted as
/// COMMENTED-OUT assertions carrying the spec value, and the divergence is stated in the
/// generated header. Each is pinned by a unit test below so the entry expires when the
/// engine is fixed.
struct Divergence {
...
fn header(out: &mut String, inject_fault: bool) {
    ...
    writeln!(out, "//").unwrap();
    writeln!(out, "// PROOF M1 (sq-3x7dl.14.2): every expected value below was READ BACK from sparq's").unwrap();
    writeln!(out, "// own Rust SPARQL/XSD scalar evaluator (`sparq_engine::query`, a real BIND over a").unwrap();
    writeln!(out, "// one-row graph) — not hand-written. Doubles are additionally cross-checked").unwrap();
    writeln!(out, "// BIT-for-BIT against native Rust f64, and fn:substring against an explicit").unwrap();
    writeln!(out, "// XPath F&O 5.4.3 window reference; a cross-check mismatch ABORTS generation.").unwrap();
    writeln!(out, "//").unwrap();
    ...
    writeln!(out, "// implements). These cases are emitted COMMENTED OUT with the SPEC value, because").unwrap();
    writeln!(out, "// asserting the oracle's answer would enshrine an engine bug and red a correct").unwrap();
    writeln!(out, "// circuit. Each is pinned by a unit test in the generator so the entry expires").unwrap();
    writeln!(out, "// when the engine is fixed:").unwrap();
...
fn gen_substring(out: &mut String, g: &Graph, c: &mut Counts) {
    ...
    writeln!(out, "/// Oracle: SPARQL `SUBSTR`, cross-checked against the F&O 5.4.3 window.").unwrap();
    writeln!(out, "#[test]").unwrap();
    ...
    for (i, &(value, cap, start, length)) in substring_corpus().iter().enumerate() {
        ...
        let prefix = if diverges { "// " } else { "" };
        if diverges {
            writeln!(out, "    // DIVERGENCE (start < 1): sparq-engine says {sparq_answer:?}, F&O 5.4.3 says").unwrap();
            writeln!(out, "    // {spec_answer:?}. Emitted COMMENTED OUT against the SPEC value — noir_XPath is").unwrap();
            writeln!(out, "    // correct here (sq-3x7dl.6) and the oracle is not. Uncomment when the engine is fixed.").unwrap();
            // One length assertion plus one per expected byte.
            c.commented_out += 1 + spec_answer.len();
        } else {
        ...
        writeln!(out, "    {prefix}let ss{i}: str<{cap}> = {};", noir_str(value, cap)).unwrap();
        writeln!(out, "    {prefix}let ({bind}, len{i}) = substring::<{cap}, {cap}>(ss{i}, {start}, {});", length.max(0)).unwrap();
        writeln!(out, "    {prefix}assert(len{i} == {});", spec_answer.len()).unwrap();
        for (j, byte) in spec_answer.as_bytes().iter().enumerate() {
            writeln!(out, "    {prefix}assert(out{i}[{j}] == {byte});").unwrap();
        }
    }
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
...
fn gen_round(out: &mut String, g: &Graph, c: &mut Counts) {
    ...
    writeln!(out, "/// preserved. Oracle: SPARQL `ROUND`, bit-cross-checked against the F&O reference.").unwrap();
    writeln!(out, "#[test]").unwrap();
    ...
    for lex in ROUND_CORPUS {
        ...
        let prefix = if diverges { "// " } else { "" };
        if diverges {
            writeln!(out, "    // DIVERGENCE: sparq-engine returns +0.0 (bits 0x{sparq_bits:016x}); F&O 4.4.3").unwrap();
            writeln!(out, "    // requires NEGATIVE zero for an argument in [-0.5, 0). Emitted COMMENTED OUT").unwrap();
            writeln!(out, "    // against the SPEC value. Uncomment when the engine is fixed.").unwrap();
            c.commented_out += 1;
        } else {
            c.assertions += 1;
        }
        ...
        writeln!(
            out,
            "    {prefix}assert(round_double(XsdDouble::from_bits({})).to_bits() == {});",
            hex64(input.to_bits()),
            hex64(spec.to_bits())
        )
        .unwrap();
...
fn generate_noir_file(inject_fault: bool) -> (String, Counts) {
    ...
    writeln!(
        out,
        "// Coverage: {} live assertions, {} commented-out (recorded oracle divergences).",
        counts.assertions, counts.commented_out
    )
    .unwrap();
...
fn main() {
    ...
    eprintln!(
        "wrote {path}: {} live assertions, {} commented-out (recorded oracle divergences)",
        counts.assertions, counts.commented_out
    );
}
...
mod tests {
    ...
    /// goes RED — which is the signal to delete the entry and uncomment those assertions.
    #[test]
    ...
    /// sign of a negative zero result. Goes RED when the engine is fixed.
    #[test]
