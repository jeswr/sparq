...
- **Provenance is sparq's differentiator**: every answer triple can carry (a) source graph/IRI, (b) for inferred triples, the rule + premises (RDF 1.2 triple-term / N3 justification). This is the antidote to "trust me" RAG. SPARQL's `GRAPH` + RDF 1.2 triple terms make provenance first-class; LLM RAG over text cannot match this.
...
| **N5** | **Provenance-carrying answers** (every answer triple → source graph + rule justification for inferred) | 4 | 2 | RDF 1.2 triple-term / N3 justification + SPARQL `GRAPH`. Directly attacks LLM hallucination distrust; auditable GenAI. Mostly plumbing on existing features. |
