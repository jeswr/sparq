...
**T3 — Handle interspersed/SPARQL-style directives instead of bailing to full serial (close the cliff).**
- Lever: today one mid-document `@prefix`/`@base` after the body starts, or any `PREFIX`/`BASE` SPARQL-style keyword, drops the **whole file** to serial (`is_sparql_directive_start` `lib.rs:2317`; bails `lib.rs:2450,2463`). Track running prefix/base state during the terminator pre-scan and emit per-chunk a snapshot of the prefix table *as of that chunk's start* (synthetic preamble), instead of only the leading preamble.
- Impact: workload-dependent — **zero** for predicate-grouped serializer output (the common Wikidata case, already fans out), meaningful for hand-written/tool-emitted Turtle that redeclares mid-file. Converts 0× cliff → ~2×.
- Risk: medium (the snapshot must be exactly right or relative-IRI/prefix resolution silently breaks — the `\#` review-1398 bug shows how subtle the splitter is). Gate: W3C TurtleTests + a fixture with interspersed `@prefix`; chunked == serial triple set.

**T4 — Switch Turtle's per-chunk dict merge to the sharded merge (lower priority).**
- Lever: `parse_turtle_chunked` merges per-chunk dicts serially (`lib.rs:2540-2544`, `merge_remap`+`remap_extend`) — the *plain* merge, not the sharded merge the external NT loader uses. On the NT path this serial merge is the identified residual bottleneck (the `Dict::merge_remap` workstream in `fast-ingestion.md`; ~200 s/1B at full scale). Use the same sharded-dict merge for Turtle.
- Impact: helps on many-core boxes; it's the same separate workstream already scoped for NT. Risk: low-medium. Gate: 8T+ scaling on a real slice.
