...
- **Phase 0 (this RFC + data):** maintainer steer on §9; run sq-6vshe.12
  ...
  *Status: llvm-lines half DONE (sq-6vshe.12 — D2 monomorphization sub-axis assessed
  above: does not veto B, the diet intervention itself has negligible surface so no
  code change shipped); the `--timings` critical-path-% half remains the open input.*
