...
- **Id substrate (`sparq-core/dict.rs`)** — PROVED (complete-domain): inline-id round-trip +
  ...
  validator never panics (today, existing lane); DIFFERENTIALLY/UNIT TESTED: full-term
  round-trips (today). TCB: Kani/CBMC.
- **Vector store (`sparq-vectors`)** — PROVED (bounded): `.spqv` open never panics on
  hostile bytes (today, existing lane). TCB: Kani/CBMC + buffer-size bounds.
