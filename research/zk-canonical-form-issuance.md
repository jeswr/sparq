...
# Proposal: canonical-form issuance conformance — the named precondition that restores INV-VL for conforming issuers
...
- `research/zk-field-native-encoding.md` §5.6 elevates "force issuers to issue only
  canonical-form values" from a future option to a **NAMED PRECONDITION** for
  relying on the dual-leaf value-FILTER lane against an adversarial issuer, and §11
  bead 5 asks to "design + implement the conformance mechanism".
...
restores INV-VL, and what it eventually buys (the leaf collapse). This record fills
that gap so the auditor (sq-qhy4) and the maintainer have one place that scopes it.
...
desynced leaf off-sparq and signs it. Canonical-form issuance conformance is the
only mechanism that closes *that* residual — for issuers that conform.
...
issuer key, attested and checkable, not a property the circuit can re-derive.** The
mechanism therefore lives at the issuance / registry / trust-graph layer, not in the
Noir circuit. That is intrinsic (§5.1): if the circuit could check it, we would not
need the precondition at all.
...
still choose to violate its own attestation (§4). What conformance buys is precise
(§4), and the docs must not overstate it.
...
- Accept a value-FILTER verdict over a dual-leaf commitment **only if** the signing
  ...
  chosen binding (§3.2). Otherwise **reject** (or downgrade to "honest-issuer-only,
  not conformance-backed", per policy) — never silently accept, matching the
...
## 4. In what sense conformance restores INV-VL (bounded, honest)

State the recovered guarantee precisely, because it is easy to overclaim:

- **Recovered:** for a credential issued by a conforming key that actually follows
  CI-DISCIPLINE, `value_component = parse(committed_lexical)` — INV-VL holds as an
  **issuance invariant**. Value identity ≡ term identity for hooked datatypes for
  that issuer class, and the desync state (§1) is unreachable *for that issuer*.
- **NOT recovered / irreducible:** conformance is a *trust* property, not a circuit
  ...
  still sign a desynced leaf; the verifier cannot detect post-commit desync
  (`zk-field-native-encoding.md` §6). So conformance moves the value lane from
  "unconditional in-circuit soundness (string-canonical)" to "sound iff the attested
  conforming issuer was honest at commit time" — a **strictly weaker** guarantee than
  string-canonical's, and a **strictly stronger, attributable** one than the bare
  dual-leaf's issuer-honesty assumption. It is defence bounding, not defence
  restoration to the string-canonical level.
- **Consequence for framing:** the value lane is **honest-issuer-only** until this
  mechanism exists, and **conformance-attested-issuer-only** after it — never
  "sound against a malicious issuer" the way string-canonical is. The gap-register
  (CR-G8) and any SKILL/README caveat must say exactly this; `check-privacy-claims.sh`
  passes because the statement is an obligation/negation, not a positive guarantee.
...
conformance for its value lane AND term identity is value-derivable for hooked
datatypes under CI-DISCIPLINE, the carried `lexical_component` hash is redundant for
those datatypes, and the leaf can collapse back toward a single value-first leaf —
recovering the commit-time cost the dual leaf pays over value-only
(`zk-field-native-encoding.md` §7). This is a **second one-time recommit** and a
separate audit-gated deliverable; it is out of scope for this proposal beyond naming
it as the payoff, and it is NOT safe for the many-to-one datatypes (`-0.0`/`+0.0`,
NaN payloads, decimal `"5.0"`/`"5.00"` at fixed scale — `zk-field-native-encoding.md`
§3.3 / §5.5) without preserving a distinct term-identity handle, so collapse is
per-datatype and conditional, never blanket.
...
- Existing `dual-leaf` commitments (once `sq-j506` exists) gain a conformance
  attestation on their issuer key retroactively via §3.1/§3.2; no leaf recommit is
  required to *attest* conformance (only to *collapse* the leaf, §5, which is a
...
before any ZK soundness/privacy/integrity property — including the INV-VL
*restoration* this proposal describes — may be relied upon. The dual-leaf method,
