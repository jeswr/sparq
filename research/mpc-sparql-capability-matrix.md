...
| Mal, HM, **minimal N=2t+1** | **EXPERIMENTAL scaffolding — NOT AUDITED** (tamper-evident-with-abort; the *malicious* cell stays **OPEN** until `sq-qhy4`) — [OPUS-5] **sq-km34** | **IT-MAC** on the equality product (NOT RS redundancy — there is none at degree 2t when n=2t+1): authenticate `[[m]] = [[d]]·[[r]]`, then MAC-check before any match bit is acted on | `auth_join::experimental_tamper_evident_secure_equal` / `experimental_tamper_evident_hidden_join`; tier reported by `auth_join::equality_join_security` as the **`SemiHonestOnly` BASELINE — NO axis promoted**, because the in-process tests evidence tamper-EVIDENCE on the deviations they exercise, not the malicious-security theorem, and external sign-off is an unresolved RELEASE GATE (`sq-qhy4`). Promoting AXIS-2 alone would NOT contain the claim — `Abort(Unanimous)` projects through the back-compat `malicious_security` to `HonestMajorityAbort`, which `is_malicious_secure()` reads as active security — so containment is asserted through every public projection by `no_public_surface_reports_active_security_before_external_sign_off`, and the abort property is evidenced as BEHAVIOUR by the tamper tests instead. The semi-honest `secure_equal` is unchanged and reports the same `SemiHonestOnly`; the reporting stays per-PATH so that a future promotion never relabels the backend. **Residual:** the MAC authenticates `m = d·r`, NOT `r ≠ 0`, so mask integrity still rests on the trusted-dealer draw (`randomness.rs` "the `r = 0` threat", exhibited by a witness test) — a dealer-less source needs an authenticated nonzero test first. Per coZK eprint 2025/1026 this cell is also a *confidentiality* hole, not just correctness; the mitigation is the check-then-act discipline at the open boundary. **Do not read this row as an audited malicious-security guarantee, and do not deploy the path as an integrity tier: `sq-qhy4` external accredited-cryptographer sign-off is PENDING.** |
...
| Mal, HM, N=3 | **EXPERIMENTAL scaffolding — NOT AUDITED** (all-pairs, tamper-evident-with-abort; the *malicious* cell stays **OPEN** until `sq-qhy4`) — [OPUS-5] **sq-km34**; SOTA still KNOWN-not-built | **ORQ 4-party Fantastic Four** sort-merge is the relational SOTA | `auth_join::experimental_tamper_evident_hidden_join` — the same all-pairs shape as the semi-honest join, with every pair's equality product IT-MAC'd and the WHOLE `\|L\|·\|R\|` batch verified in ONE `σ` open, so the IT-MAC lift costs `O(1)` rounds per join rather than per pair. Integrity only: the per-pair match bit is still opened (L2 unchanged). Tier reported AXIS-2-only (see the equality row); `sq-qhy4` external sign-off PENDING. The SOTA sort-merge shape remains **sq-ujz8 OPEN** |
...
| P2 equality | **IT-MAC** at minimal N — **BUILT as EXPERIMENTAL, unaudited scaffolding** ([OPUS-5] `sq-km34`, `auth_join`; `sq-qhy4` sign-off pending, so the malicious tier is not claimed): ~2× the multiplication work (two `mul_shares_raw` + two `degree_reduce` vs one product + one degree-`2t` open) + ~2× share volume for the MAC halves, and ONE batched `σ` open amortised over the whole batch; otherwise RS-checked (BUILT) | SPDZ MAC |
...
no-redundancy cell — **[OPUS-5] that IT-MAC has now landed for the equality/join cell as
EXPERIMENTAL, unaudited scaffolding** (`auth_join`, `sq-km34`), so P2's minimal-`N` lift is
constructed rather than projected, though the malicious tier itself remains unclaimed until
the `sq-qhy4` external sign-off; the
