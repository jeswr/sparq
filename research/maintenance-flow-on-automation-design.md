...
| New public crate → **website entry** | new `crates/<x>/Cargo.toml` (not a stub) with no `site/**` file in the same PR | a `site/src/data/surfaces.ts` entry with an honest `tier` | **F** `new-crate-site-advertisement` (shipped, #2547 — out-of-band surface) |
| New public crate → **mdBook guide page** | new `crates/<x>/Cargo.toml` (not a stub) with no `book/**` file in the same PR | a `book/src/` page listed in `SUMMARY.md` (a thin `{{#include}}` wrapper) | **F** `new-crate-guide-page` (shipped, #2547 — out-of-band surface) |
