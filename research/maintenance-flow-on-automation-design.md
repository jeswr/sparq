...
| New public crate → **test methods** beyond unit tests | new `crates/<x>/Cargo.toml` (not a `publish=false` stub) | decide + wire the applicable methods (property / differential-oracle / `fuzz/` target / Miri / Kani / mutants baseline / conformance tier / `feature-matrix.yml` leg / coverage floor) | **F** `new-crate-test-methods` (shipped, #2547 — which methods apply is a judgment call, not diff-decidable) |
| New public crate → **website entry** | new `crates/<x>/Cargo.toml` (not a stub) that `site/src/data/surfaces.ts` does not name | a `site/src/data/surfaces.ts` entry with an honest `tier` | **F** `new-crate-site-advertisement` (shipped, #2547 — out-of-band surface; emitted per added crate, suppressed only by evidence for THAT crate) |
| New public crate → **mdBook guide page** | new `crates/<x>/Cargo.toml` (not a stub) with no `SUMMARY.md`-registered page naming it | a `book/src/` page listed in `SUMMARY.md` (a thin `{{#include}}` wrapper) | **F** `new-crate-guide-page` (shipped, #2547 — out-of-band surface; emitted per added crate, suppressed only by evidence for THAT crate) |
