...
| G5 | **zk-circuit-member→gate-count** | a new member dir under `zk/compose/` (or a new circuit) | a baseline row in `crates/sparq-zk-compose/tests/gate_count_snapshot.json` (the **existing** `snapshot_covers_every_member` test already enforces this — G5 is mostly *documenting* the existing gate + extending it to top-level `zk/` circuits) | re-baseline via `bench/zk-compose/scripts/gate_counts.sh` |
...
| New **ZK circuit member** → gate-count snapshot | new member under `zk/compose/` (or new `zk/` circuit) | baseline row in `gate_count_snapshot.json` | **E/G5** (`snapshot_covers_every_member` already enforces compose members; G5 extends to top-level circuits + documents it) |
