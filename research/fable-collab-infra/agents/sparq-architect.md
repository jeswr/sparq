<!-- DRAFT — proposed .claude/agents/sparq-architect.md; staged because .claude/agents is Self-Modification-protected (AGENTS.md rule 11). Maintainer: move into .claude/agents/. [OPUS-4.8] -->
---
...
- **(c) Assign the cheapest sound tier + pin the acceptance test.** For each fragment set `model_tier` to the cheapest model that can do it soundly, write the `invariant`, and pin an `acceptance_test` that makes downstream `verify` mechanical. Anything ZK/MPC-soundness-sensitive is `opus`+ and stays maintainer-armed (see honesty).
