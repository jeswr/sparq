...
- **T2 (Wikidata <24 min):** code path ready and committed (sharded dict, zstd source,
  out-of-core build); *validation* remains hardware-blocked. Methodology + M1-measured rates:
  `research/wikidata-ingestion-benchmark.md`.
- **T1 / "T12" (many-core + NUMA):** harness ready (`sparq-cli scaling` + NUMA A/B in
  `hwrun/remote.sh`); *measurement* remains hardware-blocked. M1 curve above is the
  only datapoint; it cannot reveal NUMA effects (1 node) nor >8-thread behaviour.
...
| instance-hours | 0.0 |
| EBS/keypair/SG residue | none (cleanup verified by tag query after both attempts) |
