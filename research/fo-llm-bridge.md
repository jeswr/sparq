...
|---|---|---|
| The FO-KM **Metric-1 result** (schema.org 0.84 ≫ gUFO 0.54 < no-FO 0.58) | **MEASURED but NOT YET on `main`.** Design doc `research/foundational-ontology-km-benchmark.md` merged (PR #1106); the **harness `bench/fo-km/` + `RESULTS.md` are in OPEN PRs #1107/#1108**, not merged. The numbers exist and are cited from those PR worktrees. | The headline result is real and consistent, but this doc must cite it as *pending-merge bench record*, not a landed fact. Do not treat it as canonical until #1107/#1108 land. |
