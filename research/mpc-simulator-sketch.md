...
writes out the ideal functionalities, gives the COMPLETE open inventory of the production
semi-honest path with a per-open simulator and its exact simulation quality, composes them by a
hybrid argument with a concrete error budget, and closes with the ledger of proof obligations that
remain UNFILLED. It is NOT a proof of security and makes NO soundness claim. Date: 2026-07-28. -->
...
the model, the ideal functionalities spelled out, the **complete inventory of every value the
production semi-honest path opens**, a simulator per open with its exact simulation quality
(perfect / statistical, with the constant), the hybrid argument that composes them, and — the part
that matters most for honesty — an explicit **ledger of what the sketch does not establish**.
...
**5 distinct open shapes** on the production path.
...
## 2. The complete open inventory of the production path

An "open" is any point where shares are brought together and a field element becomes public. This
is the exhaustive list for the production semi-honest path, verified by enumerating every
`reconstruct` / `reconstruct_degree` call site outside `shamir.rs` and outside `#[cfg(test)]`:

| # | Open | Site | Degree | What becomes public | Simulation quality |
|---|------|------|--------|---------------------|--------------------|
| **A** | masked product `m = d·r`, `r` uniform **nonzero** | `join.rs::secure_equal_shared`; `compare.rs::secret_is_zero` | `2t` | one field element per test | **perfect**, from the zero/nonzero bit (§3.1) |
...
| **E** | disclosed aggregate / join result | `backend.rs::reconstruct_disclosed`; `pipeline.rs` step 5 | `t` | the intended disclosed answer | **it is the output** |
...
**Per-invocation counts** (the numbers the §4 error budget uses):

- One `disclose_threshold_verdict` run opens **64** elements: **61** × B (one per mask bit, since
  `deal_full_field_solved_bits` draws `RABBIT_MASK_BITS = 61` bits and each costs one
  square-protocol open) + **1** × C (the Rabbit masked open) + **1** × A (the `secret_is_zero`
  inside `verify_value_in_range_rabbit`) + **1** × D (the verdict).
- One scalar `HiddenValueJoin::join` over `|L|` × `|R|` rows opens **`|L|·|R|`** × A — one per
  candidate pair. This is leak **L2**, pinned by
  `join.rs::secure_equal_leaks_full_bipartite_match_graph`.
- `MpcBackend::run_secure` (the cumulative sum) opens **0**.
...
### 3.4 Lemma D/E — the output opens

`open_verdict` reconstructs `[β]_t` and refuses to coerce anything that is not exactly `0` or `1`;
`reconstruct_disclosed` opens the disclosed join/aggregate result. These are the **defined outputs**
of `F_thresh` / `F_join`, so the simulator receives them from the functionality by definition and
outputs them verbatim. There is nothing to prove — but there *is* something to state: an open is
only "free" in this sense if the functionality really does define it as an output. §3.5 is where
that bookkeeping is done, and it is where an understated functionality would hide a leak.
...
- **`F_thresh`**: on `[x]` and a public `τ`, output **two** bits — `1{x < 2^{60}}` (the in-range
  ...
  not realize.
- **`F_join`** (hidden-value): output the join, and **leak the full bipartite match graph** over
  all `|L|·|R|` pairs (leak L2) — *not* merely the output multiset. The fully-oblivious variant
...
Fix an execution of `disclose_threshold_verdict`. Define hybrids `H_0 .. H_64` where `H_0` is the
real transcript and `H_j` replaces the `j`-th open (in execution order) with its simulated value.
...
For the hidden-value join the transcript is simulated **perfectly** (`ε = 0`, all Lemma A), but
only relative to the **match-graph-leaking** `F_join`. Perfect simulation against a leaky
functionality is not a privacy result; it is a statement that the protocol leaks *exactly* what the
functionality says and nothing more. Keeping `F_join` honest is the whole content.
...
- **This is a sketch, not a proof.** It constructs simulators for the *opened values* of every
  production open and composes them by a hybrid argument. It does not construct per-party view
  simulators, and it therefore establishes **no** realization of any functionality.
