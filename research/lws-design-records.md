...
| **V4** | conditional requests (`If-Match`/`If-None-Match`) | a precondition is a content- or membership-derived validator, so a non-reader carrying one gets their denial code **instead of** the precondition being evaluated — decided *before* the existence probe, so neither the `412`-vs-`2xx` outcome nor any `ETag` is observable | `guard_conditional_requires_read` (`handler.rs:602-624`), called from PUT (`:1125`), DELETE (`:1443`), PATCH (`:1615`) |
