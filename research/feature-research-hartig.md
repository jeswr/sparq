...
- **(4c) Triple-term-aware sharded dictionary merge** (remove the `has_triple_terms`
  fallback at lib.rs:3628 so RDF-star bulk loads keep full parallelism) → **FIT:
...
(GOO + characteristic sets + WCOJ), but **no explicit well-designed-OPTIONAL handling**
(exec.rs:1047).
