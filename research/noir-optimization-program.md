...
| frequency: **501** elidable checks (≈20% of the 2516 checked unsigned `add`/`mul` ops surviving the pass today), but concentrated in **9 of 509** packages, and **0** of the `RangeCheck`-covered variant | the pattern is real (unrolled accumulators, multi-term `+` expressions) yet the existing benchmark suite would show zero movement — a focused fixture is mandatory, and the "does it pay rent" question (PR #11580 landmine) is live |
