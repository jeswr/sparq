...
| Wikidata <24 min | **Partial-scale measured; full run quota-blocked** | 1 B real triples @1.355 M/s on r7i.2xlarge `c7af391` (`research/wikidata-ingestion-benchmark.md`); the discovered 200 s/1 B serial dict bucket fixed in `d6f4033` (projected ≤153 s/1 B budget); full-scale validation needs the quota action above |
