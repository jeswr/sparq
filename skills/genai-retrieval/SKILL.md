...
// Phase 4 (default build). On PARSE FAILURE ONLY, a did-you-mean — as a DIAGNOSTIC, never
// applied: TerseError::CanaryFailed carries hints and the call still fails.
let hints = sparq_terse::keyword_hints("SELECT ?s WHERE { ?s ?p ?o } FLTR(?s > 1)");
assert_eq!(hints[0].token, "FLTR");
assert_eq!(hints[0].suggestions[0], "FILTER");     // YOU fix the query; the crate never does
...
The **did-you-mean diagnostic** (Phase 4, default build, design §3.2, sq-h7zlx) is the only
sliver of lever 2 that ships, and it is deliberately *not* lenient parsing. On a **parse
failure only**, `TerseError::CanaryFailed` carries `hints: Vec<KeywordHint { token,
suggestions }>` — *"unknown keyword `FLTR` — did you mean `FILTER`?"* — and the call **still
returns `Err` with the query unrepaired**. Nothing applies a suggestion: a lenient parse can
silently turn a typo into a *different, valid, wrong* query, and it destroys the loud,
recoverable feedback loop an agent relies on to fix its own mistake next turn. It is
conservative — a structural syntax error (unbalanced brace, missing dot) produces **no**
hints — and the keyword vocabulary is pinned to the vendored `spargebra` grammar's own
terminals by a drift test. `keyword_hints(sparql)` exposes the same diagnostic for a parse
error obtained anywhere else.
