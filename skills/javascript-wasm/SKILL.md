...
**Serialise the store (raw wasm `Store.serialize`, serialize-rdf bundle only).** Where `queryQuads` writes a CONSTRUCT/DESCRIBE *result graph* as flat N-Triples, `serialize(format, pretty, indent, abbreviate)` writes the **store itself** as a readable Turtle (default graph) or TriG (whole dataset, named graphs as `GRAPH <g> { … }` blocks) **string**. `pretty=true` gives the sorted, blank-line-separated, indented shape (the same byte shape the site's `prettyTurtle` reshaper produces); `indent` is the continuation-line unit (`null`/`undefined` ⇒ two spaces, ignored when `pretty=false`); `abbreviate=true` emits a sorted `@prefix` header and compacts IRIs to `prefix:local` (well-known prefixes `rdf`/`rdfs`/`xsd`/`owl`/`schema`/`foaf`/`dc`/`skos`/`sh`), `false` keeps full `<…>` IRIs. An unrecognised `format` throws; `'turtle'` (`'ttl'`) and `'trig'` are accepted (case-insensitive). Compiled in only with the opt-in `serialize-rdf` bundle feature (see below); the lean bundle omits it.

```js
const ttl = store.serialize('turtle', true, '  ', true); // pretty Turtle, 2-space indent, prefixes
...
- **`Store.serialize` needs the `serialize-rdf` bundle feature.** The published `@jeswr/sparq` bundle is built with `--features shacl,jsonld,serialize-rdf` (the js `build:wasm` script), so `serialize(...)` works out of the box. It is **not free in the binary** — it links `sparq-engine`'s serializer matrix (the pretty Turtle / TriG writers) — so the **lean default bundle** (`build:wasm:lean` / `cargo build -p sparq-wasm` with no `--features`) omits it. On the lean bundle the `serialize` method is absent. (The capability is the raw `Store` only for now; a high-level `SparqStore` wrapper that returns RDF/JS-friendly output is tracked separately.)
