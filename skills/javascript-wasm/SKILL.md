...
**Derive a SHACL/DASH form (`Store.deriveForm`, forms bundle only).** [GPT-5.6] The stateless raw binding parses serialized data and shapes graphs, calls `sparq_forms::derive_form`, and returns its direct snake_case `FormDescription` JSON. `focus` is `TermRef` JSON; `format` applies to both graphs; `optionsJson` is `{"mode":"edit"|"view","shape"?:TermRef}`. The explicit shape must be an IRI or blank node. The receiver's own triples are ignored, and malformed RDF, term JSON, modes, or shapes throw contextual errors.

```js
const formJson = raw.deriveForm(
  dataNQuads,
  shapesNQuads,
  JSON.stringify({ kind: 'iri', value: 'http://example.org/alice' }),
  'nquads',
  JSON.stringify({ mode: 'edit', shape: { kind: 'iri', value: 'http://example.org/PersonShape' } }),
);
const form = JSON.parse(formJson); // the direct sparq-forms FormDescription
```

The hosted GUI bundle enables `forms`; the lean/default bundle omits both the method and the `sparq-forms`/JSON dependency graph. See the [`shacl-forms` skill](../shacl-forms/SKILL.md) for the description schema, applicable-shape ranking, widgets, constraints, and nested forms.
...
- **`Store.deriveForm` needs the `forms` bundle feature.** [GPT-5.6] The hosted `build:wasm` bundle enables it so the Forms tool derives from active workspace data in-browser. It links the opt-in `sparq-forms` derivation engine and `serde_json` only in that feature state. The lean/default bundle omits the method and dependency graph, preserving its size gate. This is a raw `Store` method; no high-level `SparqStore` wrapper is added.
