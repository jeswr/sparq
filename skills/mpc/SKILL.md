...
The secure comparison opens ONLY the boolean. Reuse the secret-shared sum from recipe 1; the exact total is never reconstructed.
...
// out: one ?over_threshold = "true"^^xsd:boolean — the £108k total NEVER appears.
# Ok::<(), sparq_mpc::MpcError>(())
```
...
- **Hidden-join OUTPUT leakage (L1/L2) — mitigated by the oblivious output path.** `HiddenValueJoin::join` opens a per-pair match bit and emits the exact match count, leaking the bipartite match graph / key fan-out (L2) and the true result cardinality (L1). The `oblivious_join` module closes both for the parties: pad to a public bound `B` + oblivious-shuffle, then reveal exactly `B` shuffled slots. **Residual leakage (honest):** the parties learn only `B` (an upper bound), but the authorized result **recipient** still learns the true count by filtering dummies — this is a *padded-bound* scheme, not differential privacy. Hiding the count from the recipient too is the separate DP cardinality mode (bead sq-shk5). The output transform + the disclosed-key path are sound today; deriving the secret match bit from secret keys without opening it is gated on secure-compare (sq-rrz4/sq-dvuc).
