...
- **Secure comparison / threshold (sq-rrz4, `compare` module)** — secret-shared `>` over `Fp` that opens **only the boolean verdict bit**, never the operands or their difference (the disclosure-minimising counterpart to `reconstruct_disclosed`, which opens the integer). Bit-decomposition MSB-first comparison; chains multiplications via `mul_shares_raw` + the BGW `degree_reduce` (sq-dvuc), so it is a genuine depth-`O(L)` mult chain. **Honest-majority, semi-honest — NOT malicious** (reported as `OperatorClass::Comparison` → `SemiHonestOnly`); for the **malicious-with-abort** variant of this chain see `auth_compare` below. Operands must be `< 2^60` (`COMPARE_MAX_EXCLUSIVE`); `n >= 2t+1`; both fail-closed. Round-per-bit (LAN-fine, WAN-poor); the constant-round Rabbit/DGK/edaBits family is future work.
  ...
  - `disclose_threshold_verdict(backend, sum_shares: &[Share], public_threshold: u64) -> Result<PartialResult, MpcError>` — the four-flatmates path end-to-end: from a secret-shared sum, discloses a one-cell `?over_threshold` **boolean** partial — the exact sum is never in the output. The sum is bit-decomposed **in-MPC** (masked open of `sum + r`, never `reconstruct(sum_shares)`; sq-g7t5) with an in-protocol range proof (`sum < 2^20`, fail-closed; sq-nx0s) and a **square-protocol** mask so no party knows `r` (sq-mnv5). Honest-majority, semi-honest only.
...
> **Honesty — the sum is bit-decomposed IN-MPC (`disclose_threshold_verdict`).** The sum stays secret-shared end-to-end: there is **no** local `backend.reconstruct(sum_shares)` — that shortcut was removed (`sq-g7t5`). `disclose_threshold_verdict` bit-decomposes the *existing* sum sharing **in-MPC** via the masked-open protocol (Damgård et al. TCC'06): a random mask `[r]` is added and only the statistically-hiding `c = sum + r` is opened (gap `κ = 40` bits), then the sum's bits are recovered by a secret-shared borrow-subtraction `c ⊖ [r]`. An **in-protocol range proof** (`sq-nx0s`) proves `sum ∈ [0, 2^20)` from the recovered bits — an out-of-range sum **aborts fail-closed**, never returns a wrong verdict. **[OPUS-4.8] `sq-mnv5`** then made the mask itself deployment-grade: the solved-bits come from the **square-protocol** random-bit generator (`square_protocol_random_bit`; each bit is jointly generated, opening only `c = a²` which is independent of the bit), so **no party knows `r` or any of its bits** — the privileged in-process cleartext-dealt mask is gone. Only the verdict bit is ever an OUTPUT. **Honest scope:** still **honest-majority, semi-honest only** — the `a²`/`c = sum+r` opens and the `degree_reduce` re-sharings are unauthenticated (`sq-qhy4` external sign-off PENDING), the magnitude bound is `< 2^20` (covers the four-flatmates `10^6`), and the two residual `sq-mnv5` follow-ups (wider field / malicious MAC-checking) are open (see *Gotchas*).
...
- **In-process simulation, not a network protocol.** The "multi-party" computation runs in one process (the dealer/`HiddenValueJoin` plays all parties to deal shares and open results). There is no transport, no party-to-party messaging, no real party isolation yet. Cleartext inputs are passed to the simulator only to be shared internally. **`disclose_threshold_verdict` no longer reconstructs the sum** — that local shortcut was removed (**sq-g7t5**); the sum is bit-decomposed **in-MPC** (masked open of `c = sum + r`, then a secret-shared borrow-subtraction), with an in-protocol range proof (**sq-nx0s**) and — **[OPUS-4.8] sq-mnv5** — a **square-protocol** random-bit generator for the mask so no party knows `r`. Only the verdict bit is ever an OUTPUT. **Two sq-mnv5 deployment residuals** (`sq-qhy4` external sign-off pending): (1) **wider magnitude** beyond the `< 2^20` masked-open bound (`p = 2^61−1` forces it — needs a larger field or a non-masked-open comparison like Rabbit, eprint 2021/119) — still OPEN; (2) **malicious security** — **PARTLY CLOSED by sq-ka8m**: the malicious-with-abort *comparison chain* over secret operands now threads IT-MACs (`MacSession::auth_mul`, design §2.4 route (a)) through every gate and MAC-checks the verdict before open (`MacSession::mac_check`, §2.5), aborting on any tamper at the minimal `n = 2t+1` — see `auth_compare` (`malicious_greater_than` / `malicious_threshold`). The RESIDUAL of (2) is the end-to-end `disclose_threshold_verdict` decomposition opens (`a²`, `c = sum+r`, the zero-test products) which are not yet routed through the MAC-check.
