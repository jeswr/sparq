...
- **In-process simulation, not a network protocol.** The "multi-party" computation runs in one process (the dealer/`HiddenValueJoin` plays all parties to deal shares and open results). There is no transport, no party-to-party messaging, no real party isolation yet. Cleartext inputs are passed to the simulator only to be shared internally. **`disclose_threshold_verdict` no longer reconstructs the sum** — that local shortcut was removed (**sq-g7t5**); the sum is bit-decomposed **in-MPC** (masked open of `c = sum + r`, then a secret-shared borrow-subtraction), with an in-protocol range proof (**sq-nx0s**) and — **[OPUS-4.8] sq-mnv5** — a **square-protocol** random-bit generator for the mask so no party knows `r`. Only the verdict bit is ever an OUTPUT. **Two sq-mnv5 deployment residuals remain open** (semi-honest only throughout, `sq-qhy4` external sign-off pending): (1) **wider magnitude** beyond the `< 2^20` masked-open bound (`p = 2^61−1` forces it — needs a larger field or a non-masked-open comparison like Rabbit, eprint 2021/119); (2) **malicious security** — thread IT-MACs (`sq-km34.*`) through the decomposition/comparison chain and MAC-check the verdict before open.
