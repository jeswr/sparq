...
- `tabular <csv[.gz|.zst|.bz2]> [<name>=<csv> …] [flags]` *(opt-in `tabular` feature; [FABLE-5] sq-lsp7k.8, [SONNET-4.6] sq-u1z86)* — **materializing tabular→RDF import**, streaming end-to-end (CSV rows → first-party RFC-4180 reader → per-row N-Triples chunks → the parallel NT ingest; no whole-file buffering; compressed inputs auto-detected by extension). Two modes:
  ...
  - **R2RML** (`--mapping <r2rml.ttl>`): the materializing subset over **CSV logical tables** (`rr:tableName` binds to a CSV by file stem, or explicitly via a `<name>=<path>` positional). Supports `rr:subjectMap`/`rr:subject`, `rr:class`, `rr:predicateObjectMap`, `rr:predicateMap`/`rr:predicate`, `rr:objectMap`/`rr:object`, `rr:template`/`rr:column`/`rr:constant`, `rr:termType` (IRI/Literal/BlankNode), `rr:datatype`, `rr:language`, plus (sq-u1z86) `rr:parentTriplesMap`/`rr:joinCondition` (+`rr:child`/`rr:parent`) and `rr:graphMap`/`rr:graph`. **Fail-closed:** any other `rr:` construct — `rr:sqlQuery`, `rr:sqlVersion`, `rr:inverseExpression` (SQL virtualization is a deliberate non-goal) — is a loud exit-1 error, never a silent skip. No datatype inference here (CSV's natural datatype is string, per the spec).
  - **Joins** ([SONNET-4.6] sq-u1z86): a referencing object map `rr:objectMap [ rr:parentTriplesMap <#P> ; rr:joinCondition [ rr:child "c" ; rr:parent "p" ] ]` emits the PARENT map's subject for each parent row whose key matches the child row's. **Honest cost:** the parent table is read once up front and its generated subjects held in an in-memory keyed index (subjects only, never rows) — the child table still streams. A NULL (empty) cell on either side never joins; ZERO join conditions is the spec's cross product. Only the parent's *subject* map is evaluated, so a self-join terminates.
  - **Named graphs** ([SONNET-4.6] sq-u1z86): `rr:graphMap`/`rr:graph` on a subject map or a predicate-object map route statements to named graphs; a statement's graph set is the **union** of both, and the default graph is used when neither position carries a graph map, or when the explicit `rr:defaultGraph` constant names it. A NULL graph map is **per-position**: it withdraws only that position's contribution, so a NULL subject graph map still lets a predicate-object map's own graph apply (and vice versa), and a statement is dropped only when every graph map that applies to it generates NULL. Any graph map switches the emitted document to **N-Quads** and routes `--query` through `Graph::load_dataset`, so `GRAPH ?g { … }` sees the named graphs.
  - **Row provenance** ([SONNET-4.6] sq-u1z86): `--provenance` (valid in BOTH modes) stamps every generated row subject with `<https://sparq.dev/ns/tabular#row> "<n>"^^xsd:integer` and `<https://sparq.dev/ns/tabular#table> "<table>"`, in the subject's graph(s). Deliberately under sparq's own namespace, not `--base`, so provenance never collides with mapped data. This is how you get a row number in R2RML mode, which has no row concept ( `{_row}` stays a direct-mapping-only template placeholder, keeping R2RML templates spec-clean).
  - **Output:** default = load the graph and print the summary line; add `--query <sparql>` (+ `--format`/`--count` as in `query`) to query it in the same shot; or `--out <file.nt[.gz|.zst]>` to stream out **without building a graph** (N-Quads, and a `.nt` name warns, when graph maps are in play). `--sep <char|tab>` sets the separator. Usage errors exit 2; data/mapping errors exit 1. Fixture suite: `crates/sparq-cli/tests/fixtures/r2rml/` (W3C-R2RML-adapted cases; `expected.nq` marks a named-graph case); perf smoke: bench id `tabular-import-smoke`.
...
- **Tabular import is opt-in:** `--features tabular` adds the `tabular` subcommand (CSV direct mapping + the R2RML materializing subset, incl. `rr:parentTriplesMap` joins and `rr:graphMap` named graphs; zero new deps). SQL-connection R2RML stays out of scope and fails loudly — virtualization is a non-goal; sparq's counter-story is materializing import.
