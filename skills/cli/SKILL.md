...
- `classify <data-file> <format> [out.nt]` *(opt-in `reason-el` feature; [SONNET-4.6] sq-2ch27)* — run the **OWL 2 EL consequence-based classifier** (`sparq-reason-el`, CR1–CR6 + the `rbox` role automaton) over the ontology and print the classification summary on stdout: the concept-name count, how many `rdfs:subClassOf` / `rdfs:subPropertyOf` edges were **derived**, and every **unsatisfiable** class (`C ⊑ owl:Nothing`, named — `reason … el` only counts them). With `out.nt`, also writes the asserted triples **plus** the materialized lattice as N-Triples. **Scope of "complete":** the lattice is complete for the fragment the classifier **recognises**, assuming a **regular** RBox — not for every OWL 2 EL ontology. Axioms outside that fragment are **counted and reported on stderr, not applied**, and they come in two kinds: outside OWL 2 EL entirely (union / complement / `allValuesFrom` / cardinality / multi-individual `oneOf`), and **in-profile but not built in** — concrete domains (`owl:onDatatype` / `owl:withRestrictions`, literal `hasValue` / `oneOf`) need `sparq-reason-el/cdomain`, which the CLI's `reason-el` feature does **not** enable. A non-regular told RBox voids the completeness argument and is flagged; nominal (`owl:oneOf` / `owl:hasValue`) completeness is claimed for safe usage, not every EL++ interplay. So treat a non-zero skip count as a real incompleteness signal. **Why it exists:** OWL 2 RL (`--reason owl`) is sound but **incomplete for class classification** — it has no rule concluding membership in an intersection or reasoning through an existential successor in general, so `--reason owl` returns a hierarchy that silently omits subsumptions EL derives. RL is not an approximation you can tune up with more rules. Off by default; build with `--features reason-el` (without it, `classify` is absent and `--reason el` exits 2 with a rebuild hint rather than falling back to the incomplete RL profile).
...
```
(OWL 2 RL is sound but **incomplete for class classification** — for the `rdfs:subClassOf`
hierarchy of an EL ontology use `--reason el` / `classify`, not `--reason owl`.)

**Classify an OWL 2 EL ontology — the class hierarchy RL cannot give (opt-in `reason-el`).** Complete
for the **recognised** EL fragment under a regular RBox, *not* for the whole OWL 2 EL profile: the
feature does not enable `sparq-reason-el/cdomain`, so in-profile concrete-domain axioms are skipped
(counted + reported on stderr), and nominal completeness is claimed for safe usage only.
```bash
