...
- `dump <file[.gz|.bz2|.zst]> <in-format> <out-format>` — load an RDF document and re-serialize the whole graph (default + named graphs) to stdout in the RDF **writer matrix**. `out-format` ∈ `turtle | trig | nquads | ntriples` (Turtle emits the default graph only; trig/nquads emit the full dataset). **Opt-in:** only present when built `--features serialize-rdf` (forwards `sparq-engine/serialize-rdf`; adds zero new deps). Unknown out-format → exit 2.
