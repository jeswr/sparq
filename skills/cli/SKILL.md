...
- `tabular <csv[.gz|.zst|.bz2]> [<name>=<csv> …] [flags]` *(opt-in `tabular` feature; [FABLE-5] sq-lsp7k.8, [SONNET-4.6] sq-u1z86)* — **materializing tabular→RDF import**, streaming end-to-end (CSV rows → first-party RFC-4180 reader → per-row N-Triples chunks → the parallel NT ingest; no whole-file buffering; compressed inputs auto-detected by extension). Two modes:
  ...
  - **Named graphs** ([SONNET-4.6] sq-u1z86): `rr:graphMap`/`rr:graph` on a subject map or a predicate-object map route statements to named graphs; a statement's graph set is the **union** of both, and an empty union — or the explicit `rr:defaultGraph` constant — is the default graph. A graph map that generates NULL drops that statement. Any graph map switches the emitted document to **N-Quads** and routes `--query` through `Graph::load_dataset`, so `GRAPH ?g { … }` sees the named graphs.
