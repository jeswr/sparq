...
description: "Graph analytics over a sparq RDF graph with the opt-in sparq-algos crate: project the graph onto a directed node view (NodeGraph) and run PageRank, degree/in/out centrality, weakly-connected-component and label-propagation community detection — all read directly from sparq-core's permutation indexes, deterministic, no model, no network. Use when ranking entities by importance/centrality, finding clusters/communities, or computing connected components over a sparq Graph; topology-only (edges are predicate-erased and unweighted — filter the source graph for a per-predicate sub-graph)."
license: MIT
metadata:
  version: "0.1.0"
...
total), **weakly-connected components**, and a deterministic **label-propagation**
community heuristic. It consumes only sparq-core's public read API (the borrowing
...
`crates/sparq-algos/Cargo.toml` (it consumes `sparq-core`; no cargo features of its own):

```toml
[dependencies]
sparq-core  = { path = "../sparq-core" }
sparq-algos = { path = "../sparq-algos" }
