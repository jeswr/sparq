...
| `odrl:assignee` / `odrl:recipient` naming an `odrl:PartyCollection` (DENY head, or an ALLOW's `neq`/`isNoneOf` carve-out) | any | (none — a frozen head cannot re-check membership) | ❌ an unlisted member would escape the restriction (fail-OPEN) | **one-shot** (frozen) ([SONNET-4.6] sq-rf9uv) |
