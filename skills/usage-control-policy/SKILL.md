...
| bare `odrl:assignee` **property** (not a constraint) | — | `auth:agent <assignee>` folded into the head set (ALLOW side) | ✅ exact-IRI party — the intersection with any recipient heads (both re-check the same session agent); a PartyCollection assignee over-restricts (fail-closed — membership needs the one-shot path's request-supplied edges) | **persisted, re-checked per session** ([FABLE-5] sq-9n1q4 — previously silently dropped, widening the head to `auth:Public`). A non-trivial assignee∩recipient intersection, or an assignee spoofing `auth:Public`/`auth:Authenticated`, stays **one-shot** (fail-closed). On a **DENY** a bare assignee always stays **one-shot** (a concrete deny head can't re-check party-collection membership — members would escape, fail-open) |
| *no constraint AND no assignee* | — | `auth:agent auth:Public` (action/target/duties already held) | ✅ | persisted (public) |
