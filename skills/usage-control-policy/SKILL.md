...
**Everything else on this page has no N3 counterpart and stays Rust-only** — duties, `odrl:purpose` (and its subsumption taxonomy), `odrl:spatial`, `odrl:count` both stateless and under `count-enforcement`, `odrl:xone`, `odrl:PartyCollection`/`odrl:AssetCollection` membership, the static conflict/containment analysis, the `secprop` profile, strict `lt`/`gt` dateTime bounds, and the `odrl:use` umbrella (the rules match action, target and assignee IRIs exactly). **The driver fails closed on all of it, but *how* depends on which rule kind carries the construct** — and the difference is operationally load-bearing, because an `Err` and a successful empty authorization are not the same outcome for a caller:

- **on a permission carrying an out-of-scope *constraint*, or on the `odrl:use` umbrella** — the construct never fires a grant. For a constraint (`odrl:purpose`, `odrl:spatial`, `odrl:count`, `odrl:xone`, a strict `lt`/`gt` dateTime bound, an unsupported operator) no stratum can derive it satisfied, so no grant rule completes. For `odrl:use`, stratum A publishes an `odrlx:mode` fact only for the nine concrete actions it maps and all three grant rules in `odrl-d.n3` require `?act odrlx:mode ?mode` — so a `use` permission emits nothing even for a request whose action is *exactly* `odrl:use`. `materialize_odrl_n3` returns `Ok` having emitted no allow triple for that rule; it is **not** an `Err`. Permissions are deliberately *not* scope-guarded, because producing no grant is already fail-closed.
- **on a permission whose `odrl:target`/`odrl:assignee` names a collection** — **not** inert; do not read "outside the equivalence scope" as "never grants". The strata join the rule's target/assignee to the request's by exact IRI, so such a permission *does* grant a request naming exactly that `odrl:PartyCollection`/`odrl:AssetCollection` IRI. What has no N3 counterpart is the **membership expansion**: `evaluate` additionally matches a request whose party/asset is `odrl:partOf` the named collection, and no stratum derives that. The N3 path is a strict *subset* here rather than a blank — which is why the *N3 allows ⊆ Rust allows* inclusion still holds (`evaluate` matches on equality too).
- **on a prohibition** — the same construct makes the whole call return `Err`, materializing nothing. The Rust oracle may *satisfy* an out-of-scope prohibition constraint, so a stratum that cannot see it satisfied would drop the deny and WIDEN access; whole-policy refusal is the only fail-closed disposition. Prohibition-only refusals also cover a prohibition missing `odrl:action`/`odrl:target`/`odrl:assignee`, and a prohibition on the `odrl:use` umbrella.
- **refused regardless of rule kind** — shapes that are unsafe or ambiguous on either side: any `odrl:duty`/`odrl:obligation`; an `xsd:dateTime` outside the canonical UTC lexical subset; a rule carrying more than one logical constraint, or more than one distinct `odrl:action`/`odrl:target`/`odrl:assignee`; an ambiguous constraint node (several operand triples in one position, or several combinators); a reserved/engine-owned IRI; and an `odrl:conflict` strategy the bridge cannot honour.

Stateful `count` is out of scope by construction, not by omission: an atomic exercise-and-increment is *mutation*, and a pure inference rule cannot decrement a budget.
