...
- `GET /.well-known/void` — a [W3C VoID](https://www.w3.org/TR/void/) dataset description
  ...
  `void:classPartition` per class and `void:propertyPartition` per predicate), generated by
  `sparq-introspect`'s `Introspection::to_void`.
- `GET /sparql` **with no `query` parameter** — a [SPARQL 1.1 Service
  Description](https://www.w3.org/TR/sparql11-service-description/) (`sd:Service` with
  `sd:endpoint`, the supported `sd:supportedLanguage`s, the `sd:resultFormat`s the server
  emits, and the default dataset/graph linked to the VoID document via `dcterms:source`).
...
curl http://127.0.0.1:3030/.well-known/void                         # VoID (Turtle by default)
