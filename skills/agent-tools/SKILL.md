...
| `update` *(gated)* | `PodStore::update_as_with_budget` / `update_as_acp_with_budget` | per-graph session write check, fail-closed; bounded by the SAME per-call budget the read tools use (draft §9.4) — it reaches the check's `GRAPH ?var` binding SELECT and the apply's `… WHERE`, but not the bulk-data ops, which are costed by operand size |
