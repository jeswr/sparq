...
**CLI surfaces (opt-in `reason-el` feature, bead `sq-2ch27`, Phase E6).** The classifier is reachable
from `sparq-cli` without writing Rust. The feature is **OFF by default** — the default CLI build
carries no EL code, `classify` is absent, and `--reason el` exits 2 with a rebuild hint rather than
...
# also writes the asserted triples PLUS the materialized lattice as N-Triples.
cargo run --release -p sparq-cli --features reason-el -- classify onto.ttl turtle lattice.nt
# stdout: classified [EL]: N concept name(s) …, +M subsumption(s), +K role subsumption(s) in <t>s
#         unsatisfiable: <http://ex/A> ⊑ owl:Nothing        (named — `reason … el` only counts them)
# stderr: out-of-fragment-axiom / non-regular-RBox / unsatisfiable notes (honest, never silent)

# `--reason el` on `reason` / `query` — classify into the graph, then serialize or query it.
cargo run --release -p sparq-cli --features reason-el -- reason onto.ttl turtle el closure.nt
...
The `el` profile is **not** a `sparq_reason::Profile`: it dispatches to a different crate running a
different algorithm (consequence-based saturation, not forward-chained rules). `classify` reports
`Report::named_classes` as *concept names* — the field counts named classes **plus** ⊤/⊥ and the
anonymous (blank-node) class expressions in the normalized signature, so labelling it a named-class
total would overstate it. End-to-end coverage: `crates/sparq-cli/tests/classify_cli.rs` (the
`reason-el` feature-matrix leg), whose `owl_rl_misses_what_el_derives` case is the tripwire that
keeps the "RL cannot reach this" claim above true against sparq's own RL implementation.
...
- **OWL 2 RL is sound but INCOMPLETE for class classification.** Running `Profile::OwlRl` / `--reason owl` over an EL ontology returns a `rdfs:subClassOf` hierarchy that silently omits entailed subsumptions: the calculus never materializes an `∃r` successor and has no rule concluding membership in a class expression, so a subsumption whose derivation needs one (e.g. through a conjunction of a restriction and a named class) is unreachable — while the `scm-*` rules DO cover the easier told-restriction cases, which is why "RL misses this" must be checked, not assumed. For the class hierarchy of the **recognised EL fragment** — complete in the enabled feature set and under a regular RBox, nominals for safe usage only (see the scope caveats above) — use `sparq-reason-el` (above), from the CLI `classify` / `--reason el` behind the opt-in `reason-el` feature — not more RL rules.
