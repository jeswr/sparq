...
- **OWL 2 RL is sound but INCOMPLETE for class classification.** Running `Profile::OwlRl` / `--reason owl` over an EL ontology returns a `rdfs:subClassOf` hierarchy that silently omits entailed subsumptions: the calculus never materializes an `∃r` successor and has no rule concluding membership in a class expression, so a subsumption whose derivation needs one (e.g. through a conjunction of a restriction and a named class) is unreachable — while the `scm-*` rules DO cover the easier told-restriction cases, which is why "RL misses this" must be checked, not assumed. For the **complete** class hierarchy use `sparq-reason-el` (above) — from the CLI, `classify` / `--reason el` behind the opt-in `reason-el` feature — not more RL rules.
