...
- **Equal-atom semantics (sq-pbz04.5.4 — RIF-Core fidelity, three rules):**
  ...
  2. **Body Equal with syntactically-identical terms is eliminated** — `Equal { left: Term::Iri("x"), right: Term::Iri("x") }` (or `?n=?n`) is trivially true; `to_n3_source` omits it from the N3 body. It contributes NO bindings for range-restriction — if a head variable is SOLELY bound by a `?x=?x` atom that will be elided, `validate()` rejects the rule with `UnboundHeadVar`.
  3. **Body Equal with distinct non-variable terms is REJECTED fail-closed** — `validate()` returns `RifError::DistinctGroundEqual { left, right }`. Value-space equality (e.g. `"1"^^xsd:integer = "1.0"^^xsd:decimal`) depends on the sq-v5evr value-space comparator (issue #1646, not yet merged); the front-end refuses rather than answering incorrectly.
  4. **Variable/mixed Equal atoms (`?x=?y`, `?x=<iri>`) — KNOWN RESIDUAL DEFECT (sq-26vwp).** These lower to `owl:sameAs`-pattern triples in the N3 body. This lowering is UNSOUND: (V1) an asserted `owl:sameAs` triple in the dataset wrongly satisfies the pattern even when the two variables bind to DIFFERENT nodes (over-derivation); (V2) two variables binding to the SAME node are NOT satisfied without an explicit `owl:sameAs` assertion (under-derivation). Callers should avoid `?x=?y` / `?x=<iri>` body Equal atoms until sq-26vwp is resolved. This front-end does NOT reject them at present; it documents the defect honestly rather than claiming correctness.
...
- **W3C OWL 2 EL suite — a pinned EXTENSION ratchet (bead `sq-pbz04.2.4`, opt-in `sparq-conformance/el-suite`).** The W3C OWL WG export (`tests/w3c/owl2/all.rdf`), filtered to `test:EL` ∧ `test:RDF-BASED` (Approved, inline RDF/XML premise, no `owl:imports`), is run through the **REAL** classifier: each premise is classified with `classify_graph` (materializing the complete `rdfs:subClassOf` lattice IN PLACE) and each declared check decided — **consistency** (no unsatisfiable named class), **inconsistency** (some unsatisfiable named class — the TBox clash the classifier can see), **positive-entailment** (the lattice ENTAILS the conclusion under the SAME bnode-homomorphism `entail::entails` the RL `owl_suite` uses), **negative-entailment** (non-conclusion NOT entailed). `EL_SUITE_FLOOR` is the **MEASURED PASS count** — a **`sparq extension`** row in the central scoreboard (`scoreboard::SUITES`), tallied **separately** and **NOT** a full-OWL-2-EL-conformance claim: because the classifier is **TBox-only** (CR1–CR6, `rbox`/`cdomain` separately gated), tests needing **ABox inconsistency** (individual assertions), **RBox** property reasoning, **CR7–CR9 concrete domains**, or a conclusion in `owl:sameAs`/`owl:equivalentClass`/`owl:unionOf` axiom form are **audited PERMANENT divergences** (reported separately, **never summed into the floor**). `EL_SUITE_FLOOR` is read textually by `tests/scoreboard_floors.rs`, so the mirrored scoreboard value cannot drift; `--nocapture` prints an `OWL 2 EL ratchet pass N of M (floor F)` line the CI job `inference-conformance` re-greps.
...
- **FAIL-CLOSED CQ-shape gate (the soundness keystone) with broadened sound fragment (sq-pbz04.3.1).** PerfectRef is sound + complete only for **conjunctive queries**. The gate is **fail-closed** and now accepts a BROADENED sound fragment: **(B1)** top-level UCQ (`UNION` of CQ branches, each rewriting independently; `as_ucq`); **(B2)** RDF literal constants in role-atom object position (rigid, never `_`-eligible — the applicability condition is unchanged); **(B3)** `FILTER` over **distinguished-only** variables (passed through, fail-closed for non-distinguished vars); **(B4)** constant-only `VALUES` over distinguished variables (re-applied as an inner join, fail-closed for UNDEF / non-distinguished). **(B6)** `rdfs:subClassOf`, `rdfs:subPropertyOf`, `rdfs:domain`, `rdfs:range`, and all `owl:` predicates used as **atom predicates** are **rejected** (intensional-atom guard — fail-closed); annotation predicates (`rdfs:label/comment/seeAlso/isDefinedBy`) are admitted. Everything outside this fragment — `OPTIONAL`/`MINUS`/paths/aggregation/variable-predicate/non-distinguished FILTER or VALUES — is **rejected as `CqError::OutOfScope(reason)`**, never silently mis-answered. The applicability condition (an existential generator fires only on an UNBOUND, non-distinguished, non-shared variable) is enforced explicitly. The `reduce` MGU treats **distinguished (answer) variables as rigid** — it never identifies two answer columns.
