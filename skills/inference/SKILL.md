...
// Opt-in `profile` feature — the reasoning profiler (see the section below).
pub fn materialize_profiled(profile: Profile, dict: &mut Dict, triples: &mut Vec<[Id;3]>)
    -> (usize, profile::Report);
pub fn materialize_profiled_with(p: profile::Profiler, profile: Profile, …)
    -> (usize, profile::Report);   // pass `Profiler::with_progress(cb)` for round ticks
pub fn profile::with_profiler<R>(p: profile::Profiler, f: impl FnOnce() -> R)
    -> (R, profile::Report);       // profile ANY reasoning this thread drives, incl. incremental
...
## Reasoning profiler (opt-in `profile` feature, `sparq_reason::profile`) <!-- [SONNET-4.6] sq-6tykl.5 -->

The "which rule blew up my closure?" instrument. Install a `Profiler` around any reasoning this thread drives — `materialize_profiled(_with)` for batch, `profile::with_profiler(p, || …)` for anything else (it also covers `MaterializedGraph` / `MaterializedOwlGraph` **incremental maintenance**) — and get back a `Report`:

- `report.stats()` — a `RuleStat { rule, fired_count, derived_count, wall_time }` per rule group, in name order; `report.top(n)` is the same list ordered by offender size (emitted facts, then wall time, then name — deterministic).
- `report.rounds()` / `report.total_derived()` / `report.wall_time()` — the whole-run counters.
- `report.to_text_summary(n)` — the fixed-width top-N table a CLI prints; `report.to_json()` — the stable snake_case JSON a server/GUI renders (hand-rolled; this crate carries no serde dep).
- `Profiler::with_progress(cb)` — the **materialization progress monitor**: `cb(Progress { round, kind, derived_count, base_mutations, total_derived, elapsed })` fires once per fixpoint round of a batch materialization, and once per *incrementally maintained* insert/delete — so a long closure reports live instead of going dark. A mutation that falls back to a full re-materialization emits **no tick of its own**, and most rebuild paths (`MaterializedGraph`; `MaterializedOwlGraph` in its counting modes) re-materialize *directly* rather than calling the batch materializer, so they report **no rounds at all** — only a `MaterializedOwlGraph` already in `Fallback` mode delegates to `materialize_owl_rl` and ticks that run's rounds. Either way `incremental-rebuild` in `stats()` is what identifies the rebuild, so a silent stretch is not a stalled reasoner. The callback runs on the materializing thread with the profiler detached — keep it short and do not start nested reasoning inside it.
- **Branch on `kind` — the two tick flavours count different things.** `TickKind::Round` carries a batch round's committed facts in `derived_count` (`base_mutations == 0`). `TickKind::Insert` / `TickKind::Delete` carry the **base triples the mutation processed** in `base_mutations` (`derived_count == 0`): incremental maintenance updates derivation counts in place and never computes how many closure facts became newly visible, and a delete *removes* facts rather than deriving them, so neither is a signed derived-fact count. One asserted triple can make several closure facts visible while ticking `base_mutations: 1`. For what an incremental sweep actually emitted, read `incremental-insert` / `incremental-delete` in `stats()`.
- **Ticks are liveness; the report is the accounting.** `Progress.total_derived` is the running sum of what the *rounds* committed; `report.total_derived()` is the run's net-new-fact count — exactly what `materialize` returned. Under OWL-RL equality reasoning the two differ: the fixpoint commits only *canonical* facts, and the owl:sameAs classes are expanded back over the closure (`eq-expand`) after the last round, so a pure-equality ontology can close in one round that ticks `0` while still deriving every expanded fact. Incremental ticks move **neither** total, so a session of only inserts/deletes reports `total_derived() == 0` however much the closure churned; `rounds()` counts every tick, incremental ones included.

**Rule-group names** are the constants in `sparq_reason::profile::rules`: `schema-index`, `schema-saturate`, `abox-sweep`, `delta-sweep`, `scm-dom/rng`, `cls-*`, `commit`, `eq-sameas-merge`, `eq-expand`, `finalize`, `incremental-insert`, `incremental-delete`, `incremental-rebuild`. Pin against those constants, not against string literals.

Which groups appear tells you which *path* ran: an `owl` materialization that reports `schema-saturate` / `abox-sweep` with `rounds() == 1` took one of the OWL-RL fast paths (the ontology uses no OWL feature, or only the monotone subset) and was answered by the single-pass RDFS materializer; `delta-sweep` + `commit` over many rounds means the semi-naive fixpoint ran. A `MaterializedOwlGraph` in `Fallback` mode re-materializes on every mutation, so its `incremental-rebuild` phase encloses a whole nested batch run's groups.

**Read the numbers honestly.** Attribution is per rule **GROUP**, not per individual entailment rule: the OWL-RL semi-naive driver deliberately *fuses* rdfs2/3/5/7/9/11 with prp-inv/symp/eqp/fp/ifp/trp into one per-fact emitter (`delta-sweep`) so the sweep can fan out over rayon — splitting that into per-rule timers would change the code being measured. `derived_count` counts facts a group **emitted into the round's candidate set, before cross-rule dedup** — that is the number that finds a blow-up (a rule generating millions of duplicates is exactly the offender). It is a *candidate* count, so it is not comparable to `report.total_derived()` in either direction: duplicates push it above the net new facts, while facts created by owl:sameAs expansion are counted in `total_derived()` and emitted by no rule group. The profiler is **thread-local**: rayon worker time is attributed to the driving phase that blocked on it. Timings are wall-clock and machine-dependent — never pin an absolute duration in a test or a doc.

**Cost when off:** every hook is `cfg`'d on the feature — the `prof_*!` macros expand to *nothing* (no binding, no clock read) and the one inline accounting block is `#[cfg(feature = "profile")]` — so the default build carries zero instrumentation, and the materialized closure is unchanged in both feature states (pinned by `tests/profile.rs`, which compares a profiled run's output vector against a plain run's, element for element).
