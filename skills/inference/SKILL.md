...
- **Native-only crate.** `sparq-reason` pulls `regex` and (by default) `rayon`; it is never in the wasm dependency graph. For wasm or single-threaded builds use `default-features = false` (disables the `parallel` feature). There is no Noir/ZK toolchain requirement here — proofs are plain Rust structs.
