...
float addition is not associative the fold order is PINNED to ascending value with a
term-id tie-break, and `MIN`/`MAX` order `NaN` below `-INF` (`NaN` is not a row
failure here, unlike `FILTER`) with `+0.0`/`-0.0` an id-broken tie — so the closure is a
function of the completed lower strata, not of derivation order. Semi-naive rounds run per stratum.
