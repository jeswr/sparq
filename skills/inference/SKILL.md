...
- **Wired into the conformance entailment-regime suite as EXPERIMENTAL / OutOfScope (sq-kuvu3, opt-in `sparq-conformance/ql-experimental`).** The rewriter now runs over every `sd:EntailmentProfile pr:QL` case of `sparql11/entailment` — each conjunctive query is rewritten by `rewrite_production` and evaluated over the **unmodified data** (the DL-Lite query-rewriting semantics, no materialised closure), then compared to the suite oracle. **It is reported HONESTLY as experimental / OutOfScope, NOT a graduated conformance floor**: there is **NO `scoreboard::SUITES` row and NO ratchet `const`**, so the consolidated conformance total is byte-for-byte unchanged and a future QL regression can never silently claim OWL 2 QL conformance. Outcomes (all in the experimental OutOfScope bucket): an **ABSTAIN** when the fail-closed CQ-shape gate rejects a non-conjunctive / non-DL-Lite query (never a guess), a **computed result-equivalent** evidence row when the rewritten UCQ genuinely matches the oracle, a **computed-DIVERGENT** row when it does not (an honest gap — e.g. an intensional `?c rdfs:subClassOf …` TBox query a certain-ABOX-answer rewriter cannot answer), or an **inconclusive** setup-failure row. The runner is `sparq-conformance`'s `tests/ql_experimental_arm.rs` (opt-in `ql-experimental`; asserts the honesty invariants — every row OutOfScope, at least one fail-closed abstain — NOT a pass-count floor); the inference binary prints the QL section only with the feature on.
