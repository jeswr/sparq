...
pub enum RuleVars { N3, VarIris }  // `?x` (re-parses as the same rule, so re-running is a
    // fixpoint) | SWAP `var:` IRIs (`--pass-all-ground`: no syntactic variable survives,
    // but the rule is then constants — the grounded form is for RDF consumers, not re-reasoning)
