// [OPUS-4.8] sq-ymr2e.1 — small, stable RDF + SPARQL test-data the sibling journey suites
// (/try sq-ymr2e.2, home sq-ymr2e.3) build on. Kept deterministic and tiny: fixed IRIs, fixed
// literals, DESC-orderable numerics so a journey can assert CONCRETE result values (the "an
// answer a static site could not credibly fake" evidence, §2.1) without depending on the shipped
// sample graph, which may evolve. These are fixtures, not the product's bundled sample.
...
/** The trivial all-triples probe — the rawest editor→run→table path (matches try-query-smoke). */
