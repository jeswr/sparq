...
+ the vocabulary scope and over-claim rules (sections 12.2–12.3), the provenance-only reading
  of `zk:sourceCryptosuite` (section 15), and the relying-party audit-status obligations
  (section 17.1);
...
+ Each canonical term #strong[MUST] be encoded to one field element as
  $"Enc"_t("term") = h_2("type_code", h_s("value"))$, where $h_2$ is the Poseidon2 permutation
  over the BN254 scalar field #cite("POSEIDON2") #cite("BN06") applied to two field elements,
  $h_s$ is a Blake3 hash folded into a field element, and `type_code` is `1` for an IRI, `2`
...
+ Each canonical *triple* #strong[MUST] be encoded to one *leaf*
  $"leaf" = h_3("Enc"_t(s), "Enc"_t(p), "Enc"_t(o))$ — a single Poseidon2 permutation over the
  three term encodings.
+ The graph commitment `C(G)` #strong[MUST] be a single Poseidon2 sponge (rate 3) over the
  leaf sequence in canonical order, with a length-bearing initialisation vector giving domain
...
circuits are authored in Noir #cite("NOIR") across three source estates under `zk/` — a
vendored IEEE-754 library (`zk/ieee754`), an XPath 2.0 function library (`zk/xpath`), and the
...
#table(
  ...
  [`attributions`], [Per-pattern graph-attribution sets (which committed graph each BGP
    pattern may draw from).], [Fed to the Q6 cross-graph blank-node-join guard; enforced as a
  ...
  [`binding`], [The binding mode — `Challenge { challenge }` carrying the verifier nonce.],
...
The example below is *illustrative only*: the elided hex (`…`) is not real proof material, and
the optional privacy-upgrade members are absent. It is *not* a test vector and cannot be
verified; portable conformance fixtures are open future work (section 16).
...
```json
{
  ...
  "commitment_attestations": [
    { "commitment": "2f1e…", "issuer_public_key": "1c0aa5b7…e977",
      "signature": "…", "cryptosuite": "https://sparq.dev/ns/zk#poseidon2-schnorr-v1" }
  ],
  "attributions": [[0]],
  "entailment_regime": "Simple",
  "binding": { "Challenge": { "challenge": "0x2a" } },
  "sub_proofs": [
    { "inputs": { "Scan": { "id": "…", "commitments": ["2f1e…"] } }, "proof_hex": "…" },
    { "inputs": { "FilterInt": { "op": "Ge", "bound": 18, "expected": true } },
      "proof_hex": "…" }
  ],
...
+ The prover #strong[MUST] commit the nonce as *public-input field 0 of every sub-proof* in
  the manifest, and carry it in `binding` (`Challenge { challenge }`).
...
+ If a manifest binds a nonce other than the one issued for the request, the verifier
  ...
  #strong[MUST] still burn the issued nonce — a rejection is never a free retry. The nonce (not
...
the fail-closed discipline of sections 10.1 and 10.6 is kernel-normative (section 2.2). Failure
...
+ *full verification* (`verify_manifest`), which runs the prefilter, the structural re-checks
...
Full verification re-checks, independently of the prefilter (`sparq-zk` `verify::recheck`):
