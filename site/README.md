...
- **Measured timings** — a separate, stricter door (bead **sq-gum8.16**).
  ...
  count matches the suite's committed expected-rows (count is checked before time, so a
  ...
  via `papers/_lib/timing.typ`'s `headline_timing(key)`, which has no raw-value form — the
  provenance (aggregate, host class, commit) always renders with the number.
