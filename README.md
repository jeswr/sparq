...
<!-- sq-w9sr: the `status-caveat` anchor below is single-sourced into
     ...
     required honesty caveat should do. Its repo-relative links are made mount-portable at
     book-build time by scripts/mdbook-rewrite-links.py, EXCEPT the lowercase root-level
     `inference-conformance-report.md`, which that preprocessor does not match — so that one
     link is written absolute here to stay correct under both mounts (design record:
...
> [`inference-conformance-report.md`](https://github.com/sparq-org/sparq/blob/main/inference-conformance-report.md)).
