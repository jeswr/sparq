    ...
    "sparq engine": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "5352bae511a2e71f471bf3e569b8a10194da8854",
          "message": "docs(readme-gate): correct stale ADVISORY framing + add pre-PR readme-template guidance to agent defs (sq-bhgve) [OPUS-4.8] (#979)\n\nThe readme-template check was PROMOTED advisory→HARD under sq-8ic6\n(docs-quality.yml runs it with --enforce; the ci-summary aggregator gates on\nthe no-advisory-token job \"readme-template (template)\"). But agents kept\ntripping it (PRs #967/#969/#975/#977 all blocked) because the guidance was\nstale/missing:\n\n- scripts/check-readme-template.py header still said \"STATUS: ADVISORY … only\n  REPORTS … without blocking\" — stale + actively misleading now that it GATES.\n  Header comment + --advisory/--enforce help + the GITHUB_STEP_SUMMARY text now\n  state it is a HARD/REQUIRED gate. LOGIC UNCHANGED (self-test + --enforce still\n  green across all 36 READMEs); only comments/help/display strings touched.\n- .claude/agents/sparq-site.md + sparq-ci-infra.md gained a mandatory pre-PR\n  readme-template step (≤120 lines + 🚀 Quickstart / ✨ Features / 📚 Learn more\n  + License, or a ≤30-line <!-- internal-stub --> stub for publish=false; prefer\n  rustdoc over an over-cap README). sparq-rust-feature.md + sparq-docs.md already\n  carry equivalent guidance on origin/main, so they are unchanged.\n\nDOC-ONLY. Ref sq-8ic6.\n\nCo-authored-by: Jesse Wright <jmwright.045@gmail.com>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-20T13:30:26Z",
          "tree_id": "3e24d1269e0bde8e74a5f5be3f386c9a839a749b",
          "url": "https://github.com/jeswr/sparq/commit/5352bae511a2e71f471bf3e569b8a10194da8854"
        },
        "date": 1781963888847,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "dbe4ad898f1c6921805b75eb012aafb6b5b5858c",
          "message": "test(sparq-conformance): wire W3C JSON-LD 1.1 compact suite into the ratchet (sq-3uos5) (#969)\n\n* test(sparq-conformance): wire W3C JSON-LD 1.1 compact suite into the ratchet (sq-3uos5) [OPUS-4.8]\n\nExtends the JSON-LD conformance ratchet (sq-oy1f.2, epic sq-oy1f) with the W3C\n`compact` category, gating sparq's native hand-rolled Compaction Algorithm\n(`graph_to_jsonld_compact`, serialize-rdf) behind the opt-in `jsonld-suite`\nfeature alongside the existing toRdf + fromRdf lanes.\n\nPipeline (the REAL compaction path, oxjsonld self-reparse oracle):\n  1. parse each `jld:CompactTest` input `.jsonld` -> RDF via oxjsonld;\n  2. load that RDF into a sparq Graph (named graphs preserved);\n  3. compact against the case `@context` (top-level manifest `context` member,\n     the inner `@context` of the sibling `*-context.jsonld`);\n  4. re-parse the compacted document and require `reparse(compact(D, ctx)) === D`\n     (lossless compaction — the same round-trip invariant fromRdf uses), NOT a\n     byte diff against the suite `expect.jsonld`.\n\nHonest measured baseline at the pinned w3c/json-ld-api revision: compact\n163 pass / 58 fail / 25 skip. COMPACT_FLOOR = 163 (the actual pass count, not\ninflated). The 58 below-floor cases are real writer divergences (the\n`@type:@id`-coerced-key-vs-plain-string IRI confusion; `@graph`/`@index`/`@id`/\n`@language` multi-value container round-trips the hand-rolled writer does not yet\nreproduce), to RISE later. The 25 SKIP are negatives sparq's TOTAL compaction\ndoes not raise, JSON-LD-1.0-only cases, non-inline/remote `@context`, and\nempty-RDF inputs (SKIP is not PASS).\n\nOracle caveat (documented in the runner + README + SKILL): the comparison is\noxjsonld self-reparse equivalence, so the `@reverse` double-inversion /\nnon-string-language strict-third-party (pyld) faithfulness gap is NOT caught here\nand is tracked separately under sq-oy1f.\n\n  * `compact` GRADUATED out of the runner's NOT_IMPLEMENTED_CATS bucket (frame\n    stays deferred, sq-oy1f.6);\n  * central scoreboard `SUITES` gains the `compact` FeatureGatedCrateTest row\n    (floor 163), kept in lock-step by the `scoreboard_floors.rs` floor-sync guard\n    (new CRATE_LOCAL_FLOORS tuple + render assertion);\n  * `ci.yml` `jsonld-conformance` job adds the `^TOTAL compact ` grep ratchet\n    (COMPACT_FLOOR=163) mirroring the toRdf/fromRdf belt-and-braces gate (the\n    runner's own assert is the primary gate);\n  * no fetch-script change — the compact manifest is already vendored by the\n    same pinned clone.\n\nGates (both feature states): `cargo test -p sparq-conformance --features\njsonld-suite --test jsonld_suite` GREEN at compact >= 163; `cargo test -p\nsparq-conformance` (feature OFF) self-skip GREEN; `cargo test -p\nsparq-conformance --test scoreboard_floors` GREEN; `cargo clippy --workspace\n--all-targets --all-features -D warnings` clean; `RUSTDOCFLAGS=-D warnings cargo\ndoc --workspace --no-deps --all-features` clean. New code rustfmt-clean (the\nfile's pre-existing intentionally-deferred reformat left untouched).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* docs(sparq-conformance): tighten stub README under 30-line cap (sq-3uos5) [OPUS-4.8]\n\nThe readme-template gate (scripts/check-readme-template.py --enforce) flagged\ncrates/sparq-conformance/README.md as \"declared a stub but 35 lines (stub\ntarget: <=30)\". The crate carries the explicit `<!-- internal-stub -->`\ndirective, so the hard 30-line cap applies; this PR's compact-lane additions\npushed it to 35.\n\nCondensed prose (35 -> 26 lines) WITHOUT losing the compact-lane substance:\nmerged the three-binary bullet list into one sentence, shortened the\ncompact-lane paragraph to its essential claim, and folded the dev-only-harness\nnote + fetch-script list together. Preserved: the internal-stub directive, the\ntitle, that the opt-in `jsonld-suite` feature drives toRdf + fromRdf + compact,\nthat the compact lane (sq-3uos5) parses each `jld:CompactTest`, runs Compaction\nagainst the case `@context`, and requires lossless self-reparse\n(`reparse(compact(D, ctx)) ≡ D`), and the `## License` section.\n\nLocal checks green: check-readme-template --enforce (0 deviations) + --self-test;\ncargo build/clippy -p sparq-conformance (default + jsonld-suite, -D warnings);\ncargo test -p sparq-conformance --features jsonld-suite (compact ratchet holds\nat floor 163).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jmwright.045@gmail.com>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-20T13:49:41Z",
          "tree_id": "d6bed1c16762c588d81f4affaa19df6c0199d54e",
          "url": "https://github.com/jeswr/sparq/commit/dbe4ad898f1c6921805b75eb012aafb6b5b5858c"
        },
        "date": 1781964999110,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "4b2d5a9cce7941ffbdb0252a5184b825f0e91964",
          "message": "fix(sparq-engine): three JSON-LD 1.1 Compaction strict-third-party faithfulness gaps (sq-oy1f.12/.13/.14) [OPUS-4.8] (#978)\n\n* fix(sparq-engine): three JSON-LD 1.1 Compaction strict-third-party faithfulness gaps (sq-oy1f.12/.13/.14) [OPUS-4.8]\n\nThe W3C-Compaction writer (PR #963, behind the opt-in `serialize-rdf` feature)\nemitted three classes of document that round-trip through sparq's OWN JSON-LD\nreader (and the conformance self-reparse oracle) but are NOT faithfully\nre-expandable by a STRICT third-party processor. Each was differential-tested\nagainst the pyld W3C reference processor (`expand`/`toRdf`), exactly as the\noriginal #963 work was — the unit tests + the self-reparse oracle missed them.\n\nsq-oy1f.12 — @reverse double-invert + non-string language-map value:\n- A `@reverse`-term edge was relocated into an `@reverse` BLOCK keyed by the\n  reverse term. A reverse-term key inside an `@reverse` block double-inverts:\n  pyld applies the block's inversion AND the term's inversion and reads the\n  edge BACKWARDS (`<alice> <parent> <bob>` for source `<bob> <parent> <alice>`).\n  Fix: emit the reverse term as a FORWARD member of the object node (the\n  pyld-faithful shape — inverts exactly once). No `@reverse` block is emitted;\n  an internal sentinel carries the relocation so none leaks into the document.\n- A non-string value (typed/numeric literal) under a `@language` container was\n  put in the `@none` member — but a language map's values MUST be strings, and\n  pyld REJECTS the whole document (`language map values must be strings`). Fix:\n  route non-string values to a SEPARATE non-language key (the property IRI\n  compacted without the language term), preserving the datatype on read-back.\n  A plain string under `@none` is still valid and stays put.\n\nsq-oy1f.13 — @type:@id-coerced key vs literal IRI: a plain string literal under\na `@type:@id`/`@vocab`-coerced term read back as a node IRI (pyld turns\n`\"http://ex/x\"` into `<http://ex/x>` — a literal silently becomes a node). Fix:\nsplit literal values onto a non-coerced key; genuine node refs keep the coerced\nterm (their `{\"@id\"}` still collapses to a bare IRI).\n\nsq-oy1f.14 — multi-value @graph/@index/@id/@language container round-trips:\n- Several values sharing one `@language` slot clobbered all but the last; now\n  each language slot accumulates into an array. `@index` accumulates likewise.\n- `@id`/`@graph` containers (which fromRdf cannot losslessly populate) emitted a\n  container map pyld rejects (`illegal key … @id`); now fall back to the default\n  no-container framing, which re-expands faithfully.\n\n`compact_iri_no_list` now also skips `@id`/`@vocab` type-coerced terms (not just\ncontainer terms) so the non-coerced fallback key never collides with the coerced\nterm. The test-module reader (the conformance reparse oracle) is updated to read\nthe new shapes: a reverse-term forward member inverts; a language slot may be an\narray.\n\nTests: six permanent regression tests pinning the pyld-verified shape (not just\nsparq's own round-trip); the prior sq-oy1f.9 test (which asserted the now-fixed\ninvalid `@none:42` shape) is corrected to the faithful separate-key shape. 28\ncompaction tests + full sparq-engine lib suite green in BOTH feature states\n(serialize-rdf: 274; default: 199); clippy `--workspace --all-targets\n--all-features -D warnings` clean; rustdoc `--all-features -D warnings` clean.\n\nCONFORMANCE FLOOR: the W3C `compact` ratchet (sq-3uos5 / #969) is NOT yet on\nmain, so this PR leaves `crates/sparq-conformance` untouched; raising\nCOMPACT_FLOOR is a follow-up once #969 lands (these fixes should turn several\nbelow-floor compact cases into PASS).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* docs(sparq-engine): trim README to <=120-line template cap (readme-template gate) [OPUS-4.8]\n\nThe pyld-differential JSON-LD compaction note pushed crates/sparq-engine/README.md\nto 125 lines, over the HARD readme-template cap (scripts/check-readme-template.py,\nMAX_LINES=120). Move the detailed four-fix faithfulness note (sq-oy1f.12/.13/.14)\ninto the serialize::compact module rustdoc and leave a one-line pointer in the README;\nrewrap the RDF-writer-matrix bullet. README now 120 lines, --enforce passes; all four\nrequired sections (Quickstart/Features/Learn more/License) retained. No code behaviour\nchange (README + //! doc comment only); sparq-engine tests green in default and\nserialize-rdf feature states.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jmwright.045@gmail.com>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-20T14:03:56Z",
          "tree_id": "d9e01730b50ba3815752d034828b55fd9788d7a8",
          "url": "https://github.com/jeswr/sparq/commit/4b2d5a9cce7941ffbdb0252a5184b825f0e91964"
        },
        "date": 1781965946281,
        "tool": "customSmallerIsBetter",
        "benches": [
