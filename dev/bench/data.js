    ...
    "sparq engine": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "b86d2b483f0bafe9f2b04fd4479d114875f66aaa",
          "message": "feat(beads): SessionStart hook for bd context recovery + AGENTS.md note (#30)\n\n* feat(beads): SessionStart hook injects bd ready snapshot (context recovery) [OPUS-4.8]\n\nWires a thin .claude/settings.json SessionStart hook (scripts/bd-session-context.sh)\nthat injects the 'bd ready' list + open-bead count at session start, so a new or\npost-compaction session recovers task state automatically. Graceful no-op when bd or\n.beads/ absent. Deliberately NOT 'bd setup claude'/'bd prime' — those inject generic\nrules (no TaskCreate / no MEMORY.md) that conflict with this harness and duplicate the\nAGENTS.md beads section. Documented in AGENTS.md 'Beads session-context hook'.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* fix(beads-hook): address Copilot review — cd to repo root, drop wrong $schema, non-exhaustive type list [OPUS-4.8]\n\n- script: cd to <root> from BASH_SOURCE so the .beads check works when the hook runs\n  from a subdirectory (Copilot PR #30).\n- settings.json: remove $schema (it pointed at the JSON-Schema meta-schema, mislabeling\n  the settings file).\n- injected context: list bd types non-exhaustively + point to 'bd types' (spike etc.).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jesse@jeswr.org>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-14T17:31:58+01:00",
          "tree_id": "f9eecb029fba8b3cdf4869abe2dd3adf4c850961",
          "url": "https://github.com/jeswr/sparq/commit/b86d2b483f0bafe9f2b04fd4479d114875f66aaa"
        },
        "date": 1781454908279,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "68df131cdc4d6ca01e23a8025e522cdc8cf5c855",
          "message": "fix: roborev backlog — gitignore scope, HDT streaming sniff, SHACL README, resolve_iri test (#31)\n\n* fix(gitignore): scope *.db ignore to .beads/ (roborev 2257)\n\nThe top-level `*.db` pattern hid every .db file in the repo, risking\nsilently dropping legitimate SQLite fixtures from `git add`. Beads/Dolt\ngenerated DBs live under .beads/ (which also ships its own .gitignore),\nso scope the pattern to .beads/*.db and its journal/WAL/SHM siblings.\n\nVerified: .beads/issues.db still ignored; a root-level or crate-level\nfixture.db is now trackable.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* fix(sparq-hdt): robust magic-byte container sniff for streaming BufRead (roborev 2272)\n\nis_gzip/sniff_container assumed fill_buf() returns >=2 bytes when data is\navailable, but a streaming BufRead may legally expose 1 byte at a time, so a\ngzip/zstd/bzip2 HDT stream could be misclassified as bare HDT and fail to load.\n\nDrain a fixed 4-byte magic prefix robustly (loop read until full or EOF), then\nchain the owned prefix back onto the remaining reader for BOTH the compressed\nand the bare-HDT paths via Read::chain, so detection never depends on how the\nunderlying reader chunks data.\n\nRegression tests use a custom BufRead yielding exactly 1 byte per fill_buf:\ncontainer detection, byte-for-byte passthrough round-trip, and a short stream\nbelow the prefix length. MSRV 1.87 clean (Read::chain/Cursor are long-stable).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* docs(sparq-shacl): README — custom §6 SPARQL constraint components are implemented (roborev 2387)\n\nThe README claimed custom sh:ConstraintComponent support (sh:parameter /\nsh:validator, §6) was 'deferred' / 'out of scope'. That is stale: the feature is\nimplemented (model.rs discover_components + CustomSparql; sparql.rs ASK/SELECT\nvalidators + parameter pre-binding; eval.rs eval_custom_component) and covered by\n7 tests in tests/sparql_components.rs (ASK, SELECT, multi/optional parameters,\nkind-specific node/property validators, source-component reporting).\n\nUpdate both stale spots to describe §6 as implemented and move only the\ngenuinely-remaining limit (full sparql/pre-binding semantics: variable re-binding\nrejection, $shapesGraph) to the bead reference. Refresh the matching note in\nskills/shacl-validation/SKILL.md (which already flagged the README as stale).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* test(sparq-reason): lock resolve_iri dot-segment root-preservation (roborev 2318 FALSE-POSITIVE)\n\nFinding 2318 (Medium) claimed resolve_iri(\"http://example.org/a/b/c.n3\",\n\"../../../x\") yields \"http://example.orgx\" (popping the root past authority).\nVerified empirically: it correctly yields \"http://example.org/x\". The code\nalready guards against popping the leading root segment (parser.rs:1210-1216).\n\nAdd a focused regression test covering the exact reported case plus the RFC 3986\n§5.4 normal+abnormal dot-segment examples (incl. excess '..' clamping at root),\nso the correct behaviour is locked in. No production change — the finding is a\nfalse-positive against current HEAD.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jesse@jeswr.org>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-14T17:32:00+01:00",
          "tree_id": "fa4bfb5136fb4f7e2e85bae0d744db513085badc",
          "url": "https://github.com/jeswr/sparq/commit/68df131cdc4d6ca01e23a8025e522cdc8cf5c855"
        },
        "date": 1781455107910,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "35dc47c8c3846777488d6fcb0a63b6fd9b79f775",
          "message": "test(sparq-geo): OGC GeoSPARQL compliance ratchet + DE-9IM differential vs geo crate (sq-9h1r) (#32)\n\n* test(sparq-geo): OGC GeoSPARQL compliance ratchet + DE-9IM differential vs geo (sq-9h1r) [OPUS-4.8]\n\nTwo test additions raising confidence in crates/sparq-geo:\n\n1. OGC GeoSPARQL topology compliance RATCHET\n   (tests/ogc_compliance_ratchet.rs). A hand-curated, fixture-gated\n   pass-count floor (OGC_RATCHET_FLOOR=119) over geof:sf*/eh*/rcc8*\n   topology assertions with OGC-derived expected booleans. Mirrors the\n   SHACL W3C ratchet: assert_eq!(fail,0) + pass>=floor (floor only RISES).\n   Covers the SF (8), Egenhofer (8), RCC8 (8) families over point/line/\n   polygon operands, both operand orders, plus a WKT<->GML serialization-\n   equivalence cross-check (GeoSPARQL Sec 8.5). Honestly documents the\n   sub-conformance classes NOT covered (official CITE suite; RDFS/OWL\n   class entailment; query-rewrite property forms) -> beads sq-i2a0/sq-5ts8.\n\n2. DE-9IM DIFFERENTIAL vs the upstream geo crate\n   (tests/de9im_differential.rs). Over a seeded corpus of 500 generated\n   geometry pairs, cross-checks geof::sf_intersects/sf_contains/sf_within\n   against geo's STANDALONE Contains/Intersects predicate traits (separate\n   implementations from Relate, so a genuine differential). No\n   disagreements found. Also checks DE-9IM relation-algebra laws over the\n   corpus (disjoint==!intersects, within(a,b)==contains(b,a), equals/\n   touches symmetry, eh/rcc8 families partition region-region space) and\n   cross-checks the hand-written eh/rcc8 DE-9IM pattern strings against the\n   generic geof:relate matcher (guards the macro pattern literals).\n\nGate: cargo clippy -p sparq-geo --all-targets -D warnings clean;\ncargo test -p sparq-geo green (117 tests).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* test(geo): address Copilot review findings on PR #32 (sq-9h1r)\n\nFour documentation/correctness fixes in the new geo test files added by\nthis branch, all from the Copilot review on PR #32:\n\n1. de9im_differential.rs: module docs referenced a non-existent\n   [`KNOWN_DIVERGENCES`] intra-doc link. Replaced the dangling link with\n   prose describing the real, deliberate sfContains interior-vs-boundary\n   divergence and the load-bearing assert direction.\n2. de9im_differential.rs: the sfContains differential comment claimed the\n   geo-true/sparq-false case is \"recorded but not failed\", but no such\n   recording exists. Corrected the comment to state only the asserted\n   direction (sparq-true => geo-true) and that the asymmetric OGC case is\n   simply not asserted on (honest fix; no over-claim).\n3. ogc_compliance_ratchet.rs: GML fixtures used the gml: prefix without\n   declaring xmlns:gml. Added xmlns:gml=\"http://www.opengis.net/gml\" on\n   the root elements so the fixtures are well-formed XML. GML-parse tests\n   still pass.\n4. ogc_compliance_ratchet.rs: reworded the awkward panic message\n   \"fixture names unknown relation\" to\n   \"fixture references an unknown relation: {name}\".\n\nRe-gated: cargo test -p sparq-geo green (117 tests);\ncargo clippy -p sparq-geo --all-targets -- -D warnings clean.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jesse@jeswr.org>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-14T17:46:23+01:00",
          "tree_id": "be5e2f2fa5c5b64538cf94ac458cd776aed19e2f",
          "url": "https://github.com/jeswr/sparq/commit/35dc47c8c3846777488d6fcb0a63b6fd9b79f775"
        },
        "date": 1781455795663,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "13a45365662588e00f97e891dc57a4c7a5570b90",
          "message": "ci: PR-gating infra — ci-summary aggregator + CodeQL SAST + SHA-pin all actions; AGENTS.md directives (#34)\n\n* ci: PR-gating infra — SHA-pin all actions, ci-summary aggregator, CodeQL SAST (sq-prg4 / sq-yjoh)\n\n# [OPUS-4.8] Builds the PR-gating CI infra.\n\nSHA-pin (sq-yjoh — resolves the Scorecard Pinned-Dependencies alerts):\nevery third-party GitHub Action across .github/workflows/*.yml is now pinned to\nits full 40-hex commit SHA with a trailing `# vX.Y.Z` comment. 87 `uses:` lines,\nall 40-hex pinned; no bare tags remain (gate grep empty). Tags resolved via\n`gh api .../git/refs/tags/<tag>` (annotated tags dereferenced to the commit).\n\nNO dtolnay exception needed: dtolnay/rust-toolchain selects the toolchain from the\naction.yml CONTENT at the ref (the `stable` branch defaults the toolchain input to\n`stable`; the `1.88` branch hard-wires `toolchain: 1.88.0`), NOT from the ref name —\nverified against the action source. So pinning to the SHA of the `stable` / `1.88`\nbranch tip preserves toolchain selection. Pinned `# stable` / `# 1.88` accordingly.\n\nci-summary aggregator (sq-prg4): new ci-summary.yml — the SINGLE required status\ncheck. Since `needs:` can't span workflows, it polls the head commit's check-runs\n(`gh api repos/{repo}/commits/{sha}/check-runs`), excludes itself, waits until the\ndiscovered check-name set stabilises (stability window — handles late-registering\nworkflows + the startup race) AND all are terminal, then fails if any conclusion is\nnot success/skipped/neutral. Ported from prod-solid-server's pr-gate.yml; handles\nboth pull_request and push-to-main. permissions: checks/statuses/contents read.\n\nCodeQL SAST: new codeql.yml — github/codeql-action init+analyze for the Rust\nlanguage (GA), build-mode: none (buildless extraction), security-and-quality query\npack, on push-to-main + PR + weekly schedule, uploading to code-scanning. Resolves\nthe Scorecard SAST alert.\n\ndocs/branch-protection.md: the single required check is now `ci-summary / gate`;\nthe per-job list is demoted to an informational map (do NOT require individually).\nAdded the supply-chain + CodeQL jobs to that map, the Copilot/CodeQL review +\nconversation-resolution requirements, and a note on the SHA-pin policy + the dtolnay\nnuance. Notes CodeQL + Scorecard feed the gate/dashboard.\n\nGATE: all workflow YAML parses; bare-tag gate grep empty; 6 resolved SHAs\nspot-checked to exist upstream (checkout, dtolnay stable+1.88, codeql v3.36.2,\ngithub-action-benchmark, taiki-e cargo-llvm-cov).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* docs(agents): contribution workflow (PRs, ci-summary) + roborev review directive + clarify base gate is full-workspace [OPUS-4.8]\n\nFolds the stashed PR-workflow section into the ci-pr-gating branch and adds a\nnew directive: roborev (codex, non-Anthropic) auto-reviews every commit incl.\nworktree commits; findings are addressed by code-change-verified-on-HEAD or\nexplicit triage (fix/bead/close-with-reason), NEVER by merging the branch.\nAlso resolves codex finding (job 2422): the per-batch checklist now states the\nauthoritative pre-merge gate is full-workspace cargo test (sub-agents may scope\nin-worktree runs to affected crates).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* ci(ci-summary): unambiguous stability key + collision-proof self-exclusion\n\nAddress Copilot review on PR #34 (.github/workflows/ci-summary.yml).\n\n1. Stability key was built by comma-joining check names, but a check name\n   can itself contain a comma (e.g. \"MSRV check (Rust 1.88, declared floor)\"),\n   making the joined string ambiguous — two distinct sets could fold to the\n   same key. Join the sorted-unique names with \"\\n\" instead; a newline cannot\n   appear in a check name, so the key is unambiguous.\n\n2. Self-exclusion compared check NAME to \"gate\", which would also drop any\n   future sibling job literally named `gate`, undermining \"add/rename jobs\n   freely\". Exclude by THIS workflow run's id instead: every check-run's\n   details_url embeds .../runs/<RUN_ID>/..., and that id is ours alone. The\n   match is anchored to \"/runs/<id>\" to avoid false substring hits, with an\n   html_url fallback. Exact and collision-proof.\n\nPure gh/jq shell change; no actions added (SHA-pin invariant untouched).\nBehavior otherwise preserved (stable-empty set still passes; counts intact).\nValidated: YAML parses; jq logic exercised over self/sibling/empty/anchor/\nfallback fixtures. [OPUS-4.8]\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jesse@jeswr.org>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-14T17:46:48+01:00",
          "tree_id": "3e7be213226155d35a62626684780c2ee4d94bf8",
          "url": "https://github.com/jeswr/sparq/commit/13a45365662588e00f97e891dc57a4c7a5570b90"
        },
        "date": 1781455993578,
        "tool": "customSmallerIsBetter",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "b9de97e968a8ae3382eb40f9342575d011b1e59f",
          "message": "feat(server): SERVICE egress allowlist — default-deny SSRF guard via CLI/env/file (sq-4w18) (#35)\n\n* feat(engine): strict allowlist-only SERVICE egress policy seam (sq-4w18)\n\nAdd `with_service_egress_policy(strict, hosts, f)` and an `AllowlistOnly`\negress mode alongside the existing default-deny-private behaviour. In strict\nmode ONLY allowlisted hosts may be reached — even a public IP off the list is\nrefused before DNS resolution — so the network-exposed server can run a\ndefault-DENY-all SERVICE posture (strict + empty list = no federation at all).\n\n`with_service_egress_allow` stays as the non-strict (DenyPrivate) wrapper.\n`is_allowed` now also matches suffix-wildcard entries (\".example.org\" matches\nthe apex + any subdomain), the engine form of the server's `*.example.org`.\n\nTests: strict scope/restore, suffix-wildcard matching, and resolver-level\nstrict refusal of a public host off the list / empty-list deny-all / allowlisted\nhost dial. [OPUS-4.8]\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* feat(server): wire SERVICE egress allowlist into config (CLI/env/file) (sq-4w18)\n\nAdd an opt-in `service` cargo feature (enables sparq-engine/service) and a\ndefault-DENY-all SERVICE posture: with the feature on but no allowlist, every\nSERVICE clause is refused before any network call (SSRF guard — attacker-\ncontrolled query text can otherwise dial arbitrary hosts from the server box).\n\nConfig:\n  * `ServiceAllowlist` (new `service_config` module): exact-host + `*.suffix`\n    wildcard matcher, parse from `--service-allow` (repeatable),\n    `--service-allow-file`, and `SPARQ_SERVICE_ALLOW` (comma/ws-separated),\n    all UNIONed (additive — CLI/file only ever widen the env baseline).\n  * `ServerConfig.service_allow` field (+ env read in `from_env`).\n  * Installed via the engine's strict `with_service_egress_policy` in a new\n    `with_engine_scope` that wraps EVERY engine call (query, ASK,\n    CONSTRUCT/DESCRIBE, EXPLAIN ANALYZE, subscriptions, and the writer's\n    federated INSERT…WHERE), so the policy applies uniformly.\n\nDEFAULT POLICY: default-DENY-all SERVICE. Rationale: the server is the\nunauthenticated, network-facing surface; \"reach nothing unless explicitly\nallowlisted\" is the only safe default. Stricter than the engine's standalone\ndefault (which permits public IPs) — here even a public host must be listed.\n\nTests: ServiceAllowlist matcher unit tests (exact/wildcard/dedup/malformed,\nenv+file+CLI union), ServerConfig deny-all default + engine-entries wiring,\nand an integration test that an un-allowlisted loopback SERVICE is refused\nbefore any socket is opened (fake remote serves 0 requests) while an\nallowlisted host reaches it. [OPUS-4.8]\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* docs(http-server): SERVICE egress allowlist — SKILL.md + README (sq-4w18)\n\nUpdate the http-server SKILL.md (security note, hardening flags table rows for\n--service-allow / --service-allow-file / SPARQ_SERVICE_ALLOW, and a dedicated\n\"SERVICE federation (egress allowlist)\" gotcha) and the server README security\nposture + flags table to document the new `service` feature and the\ndefault-DENY-all SERVICE policy with rationale. Replaces the stale \"SERVICE is\nOFF / no egress filtering\" statements. [OPUS-4.8] (MAINTENANCE RULE: public CLI\nflag/env change ships its SKILL.md update in the same change.)\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* fix(server): from_env returns Result instead of panicking on bad SPARQ_SERVICE_ALLOW (#35)\n\nCopilot #4: ServerConfig::from_env is public config API an embedder may call;\na panic on a malformed SPARQ_SERVICE_ALLOW entry is a hostile surprise. Return\na clean Result<Self, String> (the crate's existing Result<_, String> error\nstyle, matching from_sources / sparq-nlq's from_env). main surfaces it via `?`\nas a one-line stderr message + non-zero exit. The valid path is unchanged.\n[OPUS-4.8] sq-4w18\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* test(server): prove no-dial under deny + short timeouts in SERVICE allowlist tests (#35)\n\nCopilot #5/#6/#7:\n- #5: the `serve` helper now returns an `accepts` channel that fires per\n  accepted connection; the deny test keeps the listener LIVE (n==1) and asserts\n  nothing is accepted within a window — proving the strict policy refuses BEFORE\n  opening a socket. Reworded the doc comment to claim only what is proven (a\n  bare count of 0 cannot distinguish \"no socket\" from a fast ECONNREFUSED).\n- #6/#7: the two deny-posture tests now set a short query_timeout (2s) instead\n  of the 30s default, so a regression that dials a (live) dead endpoint cannot\n  hang CI.\n[OPUS-4.8] sq-4w18\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n* fix(server): normalise SERVICE allow entries to the engine's bare host + RAII env guard in tests (#35)\n\nCopilot #1: an entry like `example.org:443` or a bracketed IPv6 `[::1]` was\nstored verbatim, but the engine's egress resolver compares the SERVICE IRI\nauthority with the port dropped (rsplit_once(':')) and IPv6 brackets stripped —\nso such entries silently NEVER matched (fail-open: the allowlist looked\nconfigured but was inert). `add` now normalises each exact entry to that bare\nhost. Rule: bracketed `[ipv6]` / `[ipv6]:port` -> strip the optional `:port`\nafter `]`, then strip the brackets; unbracketed `host:port` (single colon,\nall-digit port) -> drop the port; a bare unbracketed IPv6 (multiple colons) is\nleft intact so its last hextet is never amputated. New unit test covers\nexample.org:443, [::1], [::1]:443, ipv4:port, bare ::1, and full bracketed ipv6.\n\nCopilot #2/#3: the env-mutating from_sources tests now restore\nSPARQ_SERVICE_ALLOW via an RAII Drop guard (mirroring sparq-core::dictspill's\nEnvRestore) so a panic mid-test cannot leak the mutated var into sibling/other\ntests.\n[OPUS-4.8] sq-4w18\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>\n\n---------\n\nCo-authored-by: Jesse Wright <jesse@jeswr.org>\nCo-authored-by: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-14T18:26:16+01:00",
          "tree_id": "c772ccd603f18e37e53beb91298a5e4976e88bc9",
          "url": "https://github.com/jeswr/sparq/commit/b9de97e968a8ae3382eb40f9342575d011b1e59f"
        },
        "date": 1781458184215,
        "tool": "customSmallerIsBetter",
        "benches": [
