...
. "$SCRIPT_DIR/bench-s3-results.sh"
...
BENCH_S3_RUN_ID="${BENCH_S3_RUN_ID:-materialize-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
BENCH_S3_IAM_ARGS="$(bench_s3_iam_args)"
BENCH_S3_UPLOAD_LINE="$(bench_s3_userdata_line "$BENCH_S3_RUN_ID" \
  /root/sparq/bench/gather-out,/var/log/gather.log,/root/GATHER_STEP)"
