...
class DocHintsMatchLanePin(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Read the lane pin the same `with:`-scoped way the cross-workflow check does
        # (`_step_version`), so a `version:` the action never receives — under a sibling
        # `env:`, or duplicated — can never become the version the hints are held to.
        pin = _step_version(block)
        assert VERSION_RE.match(pin), (
            f"js.yml's {WASM_PACK_ACTION} step must carry exactly one exact "
            f"`with: version:` for the doc hints to be single-sourced from, got "
            f"{pin!r}"
        )
        # `cargo install --version` takes a bare semver; the action input takes `vX.Y.Z`.
        cls.pin = pin.lstrip("v")
