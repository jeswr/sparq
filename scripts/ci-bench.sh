...
# GATE MEASURES THE BINARY THAT SHIPS — js `build:wasm{,:lean}` builds the same profile via
# `wasm-pack --profile release-wasm` (size-optimised cold parse crates + strip=symbols; hot
# engine crates stay opt-level 3). Skipped gracefully when the wasm target isn't installed.
