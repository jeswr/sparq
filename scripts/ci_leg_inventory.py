...
three consequences that bound what the table means:
  ...
  3. NAME-BASED CLASSIFICATION CAN GO STALE. A renamed step silently falls to
     ...
     the live file so a renamed step reds in CI rather than in a report. Roughly a quarter
     of the step names in the OTHER workflows do not classify, because the rules below
     cover the compile/test-run distinction this bead needs and stop short of guessing at
     deploy/attest/publish steps. Pointing this tool at another workflow will often want
